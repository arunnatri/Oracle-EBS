--
-- XXD_PROJ_INV_CATEGORY_CNV_PKG  (Package Body) 
--
/* Formatted on 4/26/2023 4:27:23 PM (QP5 v5.362) */
CREATE OR REPLACE PACKAGE BODY APPS."XXD_PROJ_INV_CATEGORY_CNV_PKG"
AS
    -- +==============================================================================+
    -- +                        TOPPS Oracle 12i                                      +
    -- +==============================================================================+
    ...
    AS
        CURSOR get_category_c IS
            SELECT RECORD_ID, TRIM (style_code) style_code, TRIM (COLOR_CODE) COLOR_CODE,
                   TRIM (proj_seg1) proj_seg1, TRIM (proj_seg2) proj_seg2
              FROM XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T
             WHERE RECORD_STATUS IN ('N', 'E') AND batch_number = p_batch_no; --AND record_id = 344885
    ...
    BEGIN
        ...
        UPDATE XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T
           SET ERROR_MESSAGE   = NULL
         WHERE RECORD_STATUS IN ('N', 'E') AND batch_number = p_batch_no;


        UPDATE XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T
           SET style_code = TRIM (style_code), COLOR_CODE = TRIM (COLOR_CODE), proj_seg1 = TRIM (proj_seg1),
               proj_seg2 = TRIM (proj_seg2)
            ...
            THEN
                ...
                LOOP
                    BEGIN
                        ...
                        lc_inv_category       :=
                               TRIM (xxd_category_tab (i).proj_seg1)
                            || '.'
                            || TRIM (xxd_category_tab (i).proj_seg2);
                        ...
                        print_msg_prc (p_debug,
                                       'Category_id ' || ln_inv_category_id);
                        ...
                        THEN
                            ...
                            THEN
                                ...
                                lc_error_message   :=
                                    ...
                                    || ' Project seg1 '
                                    || xxd_category_tab (i).proj_seg1
                                    || ' is not defined for the category set PROJECT_TYPE';
                            ...
                            print_msg_prc (
                                p_debug,
                                'lc_error_flag for Season ' || lc_error_flag);
                        /*IF NOT validate_valueset_value (
                                  p_debug,
                                  'Inventory',
                                  'SEGMENT3',
                        ...
                        THEN
                           ...
                           xxd_common_utils.record_error (
                              ...
                              p_request_id   => gn_request_id, -- concurrent request ID
                              p_more_info1   => lc_inv_category, --additional information for troubleshooting
                              p_more_info2   =>    'Structure id '
                                                || ln_inv_structure_id,
                              p_more_info3   =>    'record id  '
                                                || xxd_category_tab (i).record_id);
                        END IF;

                        print_msg_prc (
                           p_debug,
                           'lc_error_flag for DEPARTMENT ' || lc_error_flag);
                        ...
                        print_msg_prc (
                           p_debug,
                           'lc_error_flag for SUB_CLASS ' || lc_error_flag);
                        ...
                        print_msg_prc (
                           p_debug,
                           'lc_error_flag for MASTER_STYLE ' || lc_error_flag);
                        ...
                        print_msg_prc (
                           p_debug,
                           'lc_error_flag for STYLE_OPTION ' || lc_error_flag); */
                        ELSE
                            lc_inv_status   := 'L';
                            --lc_inv_status := 'V';
                        ...
                        /*           lc_po_category :=
                                   ...
                                   print_msg_prc (p_debug, 'PO category ' || lc_po_category);
                                   print_msg_prc (p_debug,
                                   ...
                                   print_msg_prc (
                                      p_debug,
                                      'OM Sales Category ' || xxd_category_tab (i).STYLE);
                        ...
                        UPDATE XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T
                           SET ERROR_MESSAGE = SUBSTR (ERROR_MESSAGE || ',' || lc_error_message, 1, 1000), --BATCH_NUMBER = xxd_category_tab (i).BATCH_NUM,
                                                                                                           ...
                                                                                                           RECORD_STATUS = lc_inv_status, --OM_STRUCTURE_ID = ln_om_structure_id,
                                                                                                                                          --PO_STRUCTURE_ID = ln_po_structure_id,
                                                                                                                                          STRUCTURE_ID = ln_inv_structure_id,
                               CATEGORY_SET_ID = ln_inv_cat_set_id, --PO_CATEGORY_SET_ID = ln_po_cat_set_id,
                                                                    --OM_CATEGORY_SET_ID = ln_om_cat_set_id,
                                                                    CATEGORY_ID = ln_inv_category_id, --PO_CATEGORY_ID = ln_po_category_id,
                                                                                                      --OM_CATEGORY_ID = ln_om_category_id,
                                                                                                      --BATCH_NUMBER = xxd_category_tab (i).BATCH_NUM,
                                                                                                      creation_date = SYSDATE,
                               created_by = fnd_global.user_id, last_update_date = SYSDATE, last_updated_by = fnd_global.user_id,
                               last_update_login = fnd_global.login_id, request_id = gn_request_id
                        ...
                        THEN
                            ...
                            UPDATE XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T
                               SET ERROR_MESSAGE = ERROR_MESSAGE || ',' || lc_error_message, --BATCH_NUMBER = xxd_category_tab (i).BATCH_NUM,
                                                                                             -- om_RECORD_STATUS = 'E',
                                                                                             -- po_RECORD_STATUS = 'E',
                                                                                             RECORD_STATUS = 'E', creation_date = SYSDATE,
                                   created_by = fnd_global.user_id, last_update_date = SYSDATE, last_updated_by = fnd_global.user_id,
                                   last_update_login = fnd_global.login_id, request_id = gn_request_id
                             WHERE RECORD_ID = xxd_category_tab (i).record_id;
        ...
        SELECT COUNT (*)
          INTO ln_error_cnt
          FROM XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T
         WHERE (                                     -- om_RECORD_STATUS = 'E'
                RECORD_STATUS = 'E'--OR om_RECORD_STATUS = 'E'
                                   );
        ...
        SELECT COUNT (*)
          ...
          FROM XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T
         WHERE (                                   --  om_RECORD_STATUS <> 'E'
                RECORD_STATUS <> 'E'--AND om_RECORD_STATUS <> 'E'
                                    );
        ...
        fnd_file.put_line (
            fnd_file.OUTPUT,
               'Total no records validated in XXD_PLM_ATTR_STG_PROJ_T Table '
            || ln_val_cnt);
        fnd_file.put_line (
            fnd_file.OUTPUT,
               'Total no records Errored in XXD_PLM_ATTR_STG_PROJ_T Table '
            || ln_error_cnt);
    EXCEPTION
        WHEN OTHERS
        THEN
    ...
    BEGIN
        lc_category   :=
            p_category_rec.segment1 || '.' || p_category_rec.segment2 /*|| '.'
        ...
        THEN
            ...
            print_msg_prc (p_debug, 'l_messages ' || l_messages);
    ...
    AS
        CURSOR get_category_c IS
            SELECT RECORD_ID, STYLE_CODE, color_code,
                   proj_seg1, proj_seg2, STRUCTURE_ID,
                   --om_category_ID,
                   --po_category_ID,
                   category_ID, --om_category_set_ID,
                                --po_category_set_ID,
                                category_set_ID, --OM_RECORD_STATUS,
                                                 --PO_RECORD_STATUS,
                                                 RECORD_STATUS
              FROM XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T
             WHERE     1 = 1
                   AND (                              --OM_RECORD_STATUS = 'V'
                        --OR PO_RECORD_STATUS = 'V'
                        RECORD_STATUS = 'V')
                   AND batch_number = p_batch_no
                   AND (--  om_category_ID IS NULL
                        --                   OR po_category_ID IS NULL
                        category_ID IS NULL) /*
        ...
        IS
            SELECT category_id
             ...
             WHERE     mcb.structure_id = mcs.structure_id
                   AND mcs.category_set_name = 'PROJECT_TYPE'
    ...
    BEGIN
            ...
            THEN
                ...
                LOOP
                    BEGIN
                        ...
                        print_msg_prc (
                            p_debug,
                            'PROJECT_TYPE category creation for regular  ');
                        ...
                        IF xxd_category_tab (i).RECORD_STATUS = 'V'
                        THEN
                            l_category_rec                := NULL;
                            ...
                            l_category_rec.structure_id   :=
                                xxd_category_tab (i).STRUCTURE_ID;
                            ...
                            /* l_category_rec.segment3 :=
                             ...
                             /*    IF j = 1
                                 THEN*/
                            --End Code commented by 17-Apr-2015 as per PMO Decision
                            --l_category_rec.segment8 :=                        TRIM (xxd_category_tab (i).STYLE_OPTION);
                            --Start Code commented by 17-Apr-2015 as per PMO Decision
                            /*    || '-B';
                          ELSIF j = 2
                          THEN
                          ...
                          END IF; */
                            ...
                            --l_category_rec.attribute_category := 'Item Categories';

                            --l_category_rec.attribute5 := xxd_category_tab (i).SUB;

                            --l_category_rec.attribute6 := xxd_category_tab (i).DETAIL;

                            --l_category_rec.attribute7 :=                        xxd_category_tab (i).STYLE_CODE;

                            --l_category_rec.attribute8 :=                        xxd_category_tab (i).COLOR_CODE;
                            ...
                            l_category_rec.description    := lc_segments;
                            ...
                            THEN
                                create_category (p_debug, xxd_category_tab (i).record_id, 'PROJECT_TYPE ', l_category_rec, ln_inv_category_id, l_return_status
                                                 , l_messages);
                            ELSE
                                --lc_error_message := 'Project Category already created';
                        ...
                        /*                print_msg_prc (p_debug, 'Purchasing category creation ');
                                        ...
                                        THEN
                                           ...
                                           l_category_rec.segment2 := xxd_category_tab (i).CLASS;
                                           l_category_rec.segment3 := xxd_category_tab (i).STYLE;
                                           ...
                                           THEN
                                              ...
                                              create_category (p_debug,
                                                               ...
                                                               l_messages);
                                           ELSE
                                              lc_error_message := 'PO  Category already created';
                                           END IF;
                                           ...
                                           THEN
                                              lc_error_flag := 'Y';
                                              lc_error_message :=
                                                 lc_error_message || ' For PO ' || l_messages;
                                              lc_po_status := 'E';
                                           END IF;
                                        END IF;
                                        ...
                                        THEN
                                           ...
                                           l_category_rec.segment1 := xxd_category_tab (i).STYLE;
                                           l_category_rec.SUMMARY_FLAG := 'N';
                                           l_category_rec.ENABLED_FLAG := 'Y';

                                           l_category_rec.description := l_category_rec.segment1;

                                           lc_segments := l_category_rec.segment1;
                                           ...
                                           THEN
                                              ...
                                              create_category (p_debug,
                                                               ...
                                                               l_messages);
                                           ELSE
                                              lc_error_message := 'OM Category already created';
                                           END IF;
                                           ...
                                           THEN
                                              lc_error_flag := 'Y';
                                              lc_error_message :=
                                                 lc_error_message || ' For OM ' || l_messages;
                                              lc_om_status := 'E';
                                           END IF;
                                        END IF; */

                        UPDATE XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T
                           SET category_id = ln_inv_category_id, --om_category_id = ln_om_category_id,
                                                                 --po_category_id = ln_po_category_id,
                                                                 --om_RECORD_STATUS = lc_om_status,
                                                                 --po_RECORD_STATUS = lc_po_status,
                                                                 RECORD_STATUS = DECODE (RECORD_STATUS, 'E', 'E', lc_inv_status), ERROR_MESSAGE = ERROR_MESSAGE || ',' || lc_error_message,
                               creation_date = SYSDATE, created_by = fnd_global.user_id, last_update_date = SYSDATE,
                               last_updated_by = fnd_global.user_id, last_update_login = fnd_global.login_id, request_id = gn_request_id
                         WHERE RECORD_ID = xxd_category_tab (i).record_id;
                    --END IF;
                    EXCEPTION
                        WHEN OTHERS
                        THEN
                            ...
                            UPDATE XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T
                               SET category_id = ln_inv_category_id, --om_category_id = ln_om_category_id,
                                                                     ...
                                                                     --om_RECORD_STATUS = 'E',
                                                                     --po_RECORD_STATUS = 'E',
                                                                     RECORD_STATUS = 'E', ERROR_MESSAGE = ERROR_MESSAGE || ',' || lc_error_message,
                                   creation_date = SYSDATE, created_by = fnd_global.user_id, last_update_date = SYSDATE,
                                   last_updated_by = fnd_global.user_id, last_update_login = fnd_global.login_id, request_id = gn_request_id
                             WHERE RECORD_ID = xxd_category_tab (i).record_id;
                    END;
        ...
        SELECT COUNT (*)
          ...
          FROM XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T
         WHERE (                                     -- om_RECORD_STATUS = 'E'
                RECORD_STATUS = 'E'--OR po_RECORD_STATUS = 'E'
                                   );

        SELECT COUNT (*)
          INTO ln_val_cnt
          FROM XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T
         WHERE (                                    --  om_RECORD_STATUS = 'L'
                RECORD_STATUS = 'L'--AND po_RECORD_STATUS = 'L'
                                   );
        ...
        fnd_file.put_line (
            fnd_file.OUTPUT,
               'Total no categories created from  XXD_PLM_ATTR_STG_PROJ_T Table '
            || ln_val_cnt);
        fnd_file.put_line (
            fnd_file.OUTPUT,
               'Total no categories failed in creation from  XXD_PLM_ATTR_STG_PROJ_T Table '
            || ln_error_cnt);
    EXCEPTION
        WHEN OTHERS
        THEN
    ...
    AS
        CURSOR get_category_assign_c IS
            SELECT RECORD_ID, /*BRAND,
                   ...
                   --om_STRUCTURE_ID,
                   --po_STRUCTURE_ID,
                   STRUCTURE_ID, --om_category_ID,
                                 --po_category_ID,
                                 category_ID, --om_category_set_ID,
                                              --po_category_set_ID,
                                              category_set_ID,
                   MSIB.inventory_item_id, MSIB.organization_id, --msib.attribute28,
                                                                 --inv_s_category_id,
                                                                 --inv_b_category_id,
                                                                 --om_RECORD_STATUS,
                                                                 RECORD_STATUS
              ...
              FROM XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T XPAS, MTL_SYSTEM_ITEMS_B MSIB, ORG_ORGANIZATION_DEFINITIONS OOD
             WHERE     1 = 1
                   --AND batch_number = p_batch_no
                   AND (                              --OM_RECORD_STATUS = 'L'
                        --OR PO_RECORD_STATUS = 'L'
                        RECORD_STATUS = 'L')
                   ...
                   --AND MSIB.inventory_item_id IN (101062)
    ...
    BEGIN
            ...
            THEN
                ...
                LOOP
                    BEGIN
                        ...
                        IF xxd_category_assign_tab (i).RECORD_STATUS = 'L'
                        THEN
                            print_msg_prc (
                                p_debug,
                                   'organization_id '
                            ...
                            print_msg_prc (
                                p_debug,
                                   'inv_category_set_id '
                                || xxd_category_assign_tab (i).category_set_id);
                            ...
                            THEN
                                --print_msg_prc (p_debug,'attribute '|| xxd_category_assign_tab (i).attribute28);
                                ...
                                --End Code commented by 17-Apr-2015 as per PMO Decision
                                ln_category_id   :=
                                    xxd_category_assign_tab (i).category_id;
                                --END IF;
                        ...
                        ELSE
                            print_msg_prc (
                                p_debug,
                                   'Failed to derive old category id for PROJECT_TYPE category set for item '
                                || xxd_category_assign_tab (i).inventory_item_id);
                        END IF;
                        ...
                        /*       IF xxd_category_assign_tab (i).PO_RECORD_STATUS = 'L'
                               THEN
                                  ...
                                  OPEN get_old_category_id_c (xxd_category_assign_tab (i).organization_id,
                                                              xxd_category_assign_tab (i).inventory_item_id,
                                                              xxd_category_assign_tab (i).po_category_set_ID);

                                  ln_old_category_id := NULL;

                                  FETCH get_old_category_id_c INTO ln_old_category_id;

                                  CLOSE get_old_category_id_c;

                                  print_msg_prc (
                                     p_debug,
                                     'PO ln_old_category_id ' || ln_old_category_id);



                                  IF NVL (ln_old_category_id, 1) <>
                                        NVL (xxd_category_assign_tab (i).po_category_id,
                                             2)
                                  THEN
                                     Update_Category_Assignment (
                                        p_debug,
                               ...
                               print_msg_prc (p_debug, 'lc_error_flag ' || lc_error_flag);
                       */
                        ...
                        UPDATE XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T
                           SET ERROR_MESSAGE = SUBSTR (lc_error_message, 1, 2000), creation_date = SYSDATE, created_by = fnd_global.user_id,
                               ...
                               --om_RECORD_STATUS = lc_om_status,
                               --po_RECORD_STATUS = lc_po_status,
                               RECORD_STATUS = DECODE (RECORD_STATUS, 'E', 'E', lc_inv_status), request_id = gn_request_id
                         WHERE RECORD_ID =
                               xxd_category_assign_tab (i).record_id;
        ...
        SELECT COUNT (*)
          INTO ln_val_cnt
          FROM XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T
         WHERE (                                    --  om_RECORD_STATUS = 'P'
                RECORD_STATUS = 'P'--AND om_RECORD_STATUS = 'P'
                                   );
        ...
        fnd_file.put_line (
            fnd_file.OUTPUT,
               'Total no of record processed for assignment from  XXD_PLM_ATTR_STG_PROJ_T Table '
            || ln_val_cnt);
        fnd_file.put_line (
            fnd_file.OUTPUT,
               'Total no of record failed for assignment from  XXD_PLM_ATTR_STG_PROJ_T Table '
            || ln_error_cnt);
    ...
    IS
        ...
        CURSOR Get_batch_num_c IS
            SELECT DISTINCT batch_number
             ...
             WHERE (                                     --record_status = 'L'
                    record_status = 'L'---record_status = 'L'
                                       );
        ...
        CURSOR Get_batch_num_cr_c IS
            SELECT DISTINCT batch_number
              FROM XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T
             WHERE (                                --  om_record_status = 'V'
                    record_status = 'V'--OR po_record_status = 'V'
                                       );
        ...
        CURSOR Get_batch_num_val_c IS
            SELECT DISTINCT batch_number
              FROM XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T
             WHERE (                         -- om_record_status IN ('N', 'E')
                    record_status IN ('N', 'E')--OR po_record_status IN ('N', 'E')
                                               );



        /*     CURSOR Get_non_plm_data_c
             IS
                SELECT style_code,
                       BRAND,
                       CLASS,
    ...
    BEGIN
              ...
              LOOP
                 ...
                 LOOP
                    ...
                    INSERT INTO XXD_CONV.XXD_PLM_ATTR_STG_T (record_id,
                                                             ...
                                                             PO_RECORD_STATUS,
                                                             OM_RECORD_STATUS)
                         VALUES (XXD_CONV.XXD_PLM_ATTR_STG_SEQ.NEXTVAL,
                                 'NONPLM',
                                 lcu_Get_non_plm_data_c.BRAND,
                                 lcu_Get_non_plm_data_c.CLASS,
                                 lcu_inv_data.color_code,
                                 lcu_Get_non_plm_data_c.DEPARTMENT,
                                 lcu_Get_non_plm_data_c.DIVISION,
                                 lcu_Get_non_plm_data_c.MASTER_STYLE,
                                 lcu_Get_non_plm_data_c.STYLE,
                                 lcu_Get_non_plm_data_c.STYLE_CODE,
                                 lc_style_option,
                                 lcu_Get_non_plm_data_c.SUB_CLASS,
                                 lcu_Get_non_plm_data_c.SUB,
                                 lcu_Get_non_plm_data_c.DETAIL,
                                 'N',
                                 'N',
                                 'N');
                 END LOOP;
        ...
        THEN
            ...
            UPDATE Xxd_Conv.XXD_PLM_ATTR_STG_PROJ_T
               SET PROJ_SEG1 = UPPER (SUBSTR (PROJ_SEG1, 1, 40)), PROJ_SEG2 = UPPER (SUBSTR (PROJ_SEG2, 1, 40)), Record_Status = 'N',
                   Batch_Number = NULL, Structure_Id = NULL, Category_Id = NULL,
                   error_message = NULL, CATEGORY_SET_ID = NULL, RECORD_ID = NULL;

            --fnd_file.put_line (fnd_file.LOG, 'Test2');

            COMMIT;

            INSERT INTO XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T (RECORD_ID, STYLE_CODE, COLOR_CODE, proj_seg1, proj_seg2, /*BRAND,
                                                                                                                   DIVISION,
                                                                                                                   DEPARTMENT,
                                                                                                                   CLASS,
                                                                                                                   SUB_CLASS,
                ...
                SELECT XXD_CONV.XXD_PLM_ATTR_STG_SEQ.NEXTVAL, --    Commented on 09-OCT-2015
                       ...
                       proj_seg1, proj_seg2, /*   BRAND,
                                             ...
                                             RECORD_STATUS,
                       --PO_RECORD_STATUS,
                       NTILE (p_no_of_process) OVER (ORDER BY proj_seg1, proj_seg2)
                  FROM XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T;
            ...
            DELETE XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T
            ...
            UPDATE XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T X2
               SET X2.batch_number   =
                       (SELECT MIN (batch_number)
                          FROM XXD_CONV.XXD_PLM_ATTR_STG_PROJ_T X1
                         WHERE     NVL (X1.proj_seg1, 'XX') =
                                   NVL (X2.proj_seg1, 'XX')
                               AND NVL (X1.proj_seg2, 'XX') =
                                   NVL (X2.proj_seg2, 'XX'));
            ...
            LOOP
                ...
                ln_request_id   :=
                    fnd_request.submit_request (application => 'XXDCONV', program => 'XXD_PROJ_CAT_VAL', description => 'Deckers Project Item Categories Validation  Program -Child', start_time => SYSDATE, sub_request => NULL, argument1 => ln_batch_no
                                                , argument2 => p_debug);
        ...
        THEN
            ...
            LOOP
                ...
                ln_request_id   :=
                    fnd_request.submit_request (application => 'XXDCONV', program => 'XXD_PROJ_CAT_CREATE', description => 'Deckers Project Item Categories Creation  Program -Child', start_time => SYSDATE, sub_request => NULL, argument1 => ln_batch_no
                                                , argument2 => p_debug);
        ...
        THEN
            ...
            LOOP
                ...
                ln_request_id   :=
                    fnd_request.submit_request (application => 'XXDCONV', program => 'XXD_PROJ_CAT_UPDATE', description => 'Deckers Project Item Categories Creation and Assignment Program -Child', start_time => SYSDATE, sub_request => NULL, argument1 => ln_batch_no
                                                , argument2 => p_debug);
...
END Xxd_Proj_Inv_Category_Cnv_Pkg;
/
