--
-- XXDO_PO_ASN_RECEIPT_HEAD_STG  (Table) 
--
CREATE TABLE XXDO.XXDO_PO_ASN_RECEIPT_HEAD_STG
(
  WH_ID                  VARCHAR2(10 BYTE)      NOT NULL,
  APPOINTMENT_ID         VARCHAR2(30 BYTE),
  RECEIPT_DATE           DATE,
  EMPLOYEE_ID            VARCHAR2(10 BYTE),
  EMPLOYEE_NAME          VARCHAR2(100 BYTE),
  PROCESS_STATUS         VARCHAR2(20 BYTE)      NOT NULL,
  ERROR_MESSAGE          VARCHAR2(1000 BYTE),
  REQUEST_ID             NUMBER                 NOT NULL,
  CREATION_DATE          DATE                   NOT NULL,
  CREATED_BY             NUMBER                 NOT NULL,
  LAST_UPDATE_DATE       DATE                   NOT NULL,
  LAST_UPDATED_BY        NUMBER                 NOT NULL,
  SOURCE_TYPE            VARCHAR2(20 BYTE),
  ATTRIBUTE1             VARCHAR2(50 BYTE),
  ATTRIBUTE2             VARCHAR2(50 BYTE),
  ATTRIBUTE3             VARCHAR2(50 BYTE),
  ATTRIBUTE4             VARCHAR2(50 BYTE),
  ATTRIBUTE5             VARCHAR2(50 BYTE),
  ATTRIBUTE6             VARCHAR2(50 BYTE),
  ATTRIBUTE7             VARCHAR2(50 BYTE),
  ATTRIBUTE8             VARCHAR2(50 BYTE),
  ATTRIBUTE9             VARCHAR2(50 BYTE),
  ATTRIBUTE10            VARCHAR2(50 BYTE),
  ATTRIBUTE11            VARCHAR2(50 BYTE),
  ATTRIBUTE12            VARCHAR2(50 BYTE),
  ATTRIBUTE13            VARCHAR2(50 BYTE),
  ATTRIBUTE14            VARCHAR2(50 BYTE),
  ATTRIBUTE15            VARCHAR2(50 BYTE),
  ATTRIBUTE16            VARCHAR2(50 BYTE),
  ATTRIBUTE17            VARCHAR2(50 BYTE),
  ATTRIBUTE18            VARCHAR2(50 BYTE),
  ATTRIBUTE19            VARCHAR2(50 BYTE),
  ATTRIBUTE20            VARCHAR2(50 BYTE),
  SOURCE                 VARCHAR2(20 BYTE)      DEFAULT 'ORDER'               NOT NULL,
  DESTINATION            VARCHAR2(20 BYTE)      NOT NULL,
  RECORD_TYPE            VARCHAR2(20 BYTE)      DEFAULT 'EBS'                 NOT NULL,
  RECEIPT_HEADER_SEQ_ID  NUMBER,
  ORGANIZATION_ID        NUMBER
)
TABLESPACE CUSTOM_TX_TS
PCTUSED    0
PCTFREE    10
INITRANS   1
MAXTRANS   255
STORAGE    (
            INITIAL          64K
            NEXT             1M
            MINEXTENTS       1
            MAXEXTENTS       UNLIMITED
            PCTINCREASE      0
            BUFFER_POOL      DEFAULT
           )
LOGGING 
NOCOMPRESS 
NOCACHE
/


--
-- XXDO_PO_ASN_RCPT_HEAD_STG_IDX1  (Index) 
--
--  Dependencies: 
--   XXDO_PO_ASN_RECEIPT_HEAD_STG (Table)
--
CREATE INDEX XXDO.XXDO_PO_ASN_RCPT_HEAD_STG_IDX1 ON XXDO.XXDO_PO_ASN_RECEIPT_HEAD_STG
(RECEIPT_HEADER_SEQ_ID)
LOGGING
TABLESPACE CUSTOM_TX_TS
PCTFREE    10
INITRANS   2
MAXTRANS   255
STORAGE    (
            INITIAL          64K
            NEXT             1M
            MINEXTENTS       1
            MAXEXTENTS       UNLIMITED
            PCTINCREASE      0
            BUFFER_POOL      DEFAULT
           )
/

--
-- XXDO_PO_ASN_RCPT_HEAD_STG_IDX2  (Index) 
--
--  Dependencies: 
--   XXDO_PO_ASN_RECEIPT_HEAD_STG (Table)
--
CREATE INDEX XXDO.XXDO_PO_ASN_RCPT_HEAD_STG_IDX2 ON XXDO.XXDO_PO_ASN_RECEIPT_HEAD_STG
(WH_ID, APPOINTMENT_ID)
LOGGING
TABLESPACE CUSTOM_TX_TS
PCTFREE    10
INITRANS   2
MAXTRANS   255
STORAGE    (
            INITIAL          64K
            NEXT             1M
            MINEXTENTS       1
            MAXEXTENTS       UNLIMITED
            PCTINCREASE      0
            BUFFER_POOL      DEFAULT
           )
/

--
-- XXDO_PO_ASN_RCPT_HEAD_STG_IDX3  (Index) 
--
--  Dependencies: 
--   XXDO_PO_ASN_RECEIPT_HEAD_STG (Table)
--
CREATE INDEX XXDO.XXDO_PO_ASN_RCPT_HEAD_STG_IDX3 ON XXDO.XXDO_PO_ASN_RECEIPT_HEAD_STG
(PROCESS_STATUS)
LOGGING
TABLESPACE CUSTOM_TX_TS
PCTFREE    10
INITRANS   2
MAXTRANS   255
STORAGE    (
            INITIAL          64K
            NEXT             1M
            MINEXTENTS       1
            MAXEXTENTS       UNLIMITED
            PCTINCREASE      0
            BUFFER_POOL      DEFAULT
           )
/

--
-- XXDO_PO_ASN_RECEIPT_HEAD_STG  (Synonym) 
--
--  Dependencies: 
--   XXDO_PO_ASN_RECEIPT_HEAD_STG (Table)
--
CREATE OR REPLACE SYNONYM APPS.XXDO_PO_ASN_RECEIPT_HEAD_STG FOR XXDO.XXDO_PO_ASN_RECEIPT_HEAD_STG
/


GRANT ALTER, DELETE, INDEX, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, READ, DEBUG, FLASHBACK ON XXDO.XXDO_PO_ASN_RECEIPT_HEAD_STG TO SOA_INT
/
