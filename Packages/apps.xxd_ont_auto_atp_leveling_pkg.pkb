--
-- XXD_ONT_AUTO_ATP_LEVELING_PKG  (Package Body) 
--
/* Formatted on 4/26/2023 4:29:06 PM (QP5 v5.362) */
CREATE OR REPLACE PACKAGE BODY APPS."XXD_ONT_AUTO_ATP_LEVELING_PKG"
AS
    -- ####################################################################################################################
    -- Package      : xxd_ont_auto_atp_leveling_pkg
    ...
    -- 26-Jan-2022    Shivanshu Talwar       1.2    Modified w.r.t CCR CCR0009825
    -- 30-Aug-2022    Shivanshu Talwar       1.3    Modified w.r.t CCR CCR0010185
    -- #########################################################################################################################

    --Global Variables declaration
    gv_package_name      VARCHAR2 (200) := 'XXD_ONT_AUTO_ATP_LEVELING_PKG';
    ...
    IS
        CURSOR report_cur IS
            ...
            SELECT hou.name operating_unit,
                   ...
                   oola.creation_date,
                   oota.name order_type,
                   oola.line_number || '.' || shipment_number line_num,
                   hp.party_name customer_name,
                   hca.account_number,
                   (SELECT jrre.resource_name
                      FROM oe_order_lines_all ol, jtf_rs_salesreps jrs, jtf_rs_resource_extns_tl jrre
             ...
             WHERE     1 = 1
                   ...
                   AND xna.header_id = ooha.header_id
                   AND xna.header_id = oola.header_id
                   AND xna.line_id = oola.split_from_line_id
            ORDER BY item_number, request_date, schedule_ship_date;
    ...
    PROCEDURE get_atp_qty (
        p_line_id                     IN            NUMBER,
        p_move                        IN            VARCHAR2,
        x_atp_current_available_qty      OUT NOCOPY NUMBER,
    ...
    BEGIN
        ...
        IF p_move = 'UNSCHEDULE_MOVE'
        THEN
            l_atp_rec.requested_ship_date (1)   :=
                l_line_rec.latest_acceptable_date;
        ELSE
            l_atp_rec.requested_ship_date (1)   := l_line_rec.request_date;
        END IF;
    ...
    IS
        ...
        (
            ...
            sf_ex_flag                VARCHAR2 (10),
            ar_ex_flag                VARCHAR2 (10),
            un_ex_flag                VARCHAR2 (10)
        );
            ...
            BEGIN
                ...
                THEN
                    ...
                    lv_process_move_cond   :=
                        ' AND sf_ex_flag =''Y''  AND NVL(SPLIT_CASE,''N'') =''N''';
                    lv_order_move          := 'SAFE_MOVE';  --w.r.t CCR0009825
                ELSIF pv_order_move = 'ATRISK_MOVE'
                THEN
                    lv_process_move_cond   :=
                        ' AND ar_ex_flag =''Y''  AND NVL(SPLIT_CASE,''N'') =''N''';
                    lv_unschedule   := 'N';
                    lv_order_move   := 'ATRISK_MOVE';       --w.r.t CCR0009825
                ELSIF pv_order_move = 'UNSCHEDULE_MOVE'
                THEN
                    lv_process_move_cond   :=
                        ' AND un_ex_flag =''Y''   AND NVL(SPLIT_CASE,''N'') =''N''';
                    lv_unschedule   := 'Y';
                    lv_order_move   := 'UNSCHEDULE_MOVE';   --w.r.t CCR0009825
                ELSIF pv_order_move = 'SPLIT_CASE'
                THEN
                    -- lv_process_move_cond :=' AND un_ex_flag =''Y''   AND NVL(SPLIT_CASE,''N'') =''Y''';
                    lv_process_move_cond   :=
                        '  AND (un_ex_flag =''Y'' OR ar_ex_flag =''Y'' OR sf_ex_flag =''Y'')  AND NVL(SPLIT_CASE,''N'') =''Y''';
                    lv_unschedule   := 'Y';
                    lv_order_move   := 'SPLIT_CASE';        --w.r.t CCR0009825
                END IF;
                ...
                THEN
                    ...
                    LOOP
                        ...
                        IF     pv_order_move = 'SPLIT_CASE'
                           AND NVL (so_line_rec (i).sf_ex_flag, 'N') = 'Y'
                           AND NVL (so_line_rec (i).split_case, 'N') = 'Y' --start w.r.t CCR0009825
                        THEN
                            lv_unschedule   := 'N';
                            lv_order_move   := 'SAFE_MOVE';
                        ELSIF     pv_order_move = 'SPLIT_CASE'
                              AND NVL (so_line_rec (i).ar_ex_flag, 'N') = 'Y'
                              AND NVL (so_line_rec (i).split_case, 'N') = 'Y'
                        THEN
                            lv_unschedule   := 'N';
                            lv_order_move   := 'ATRISK_MOVE';
                        ELSIF     pv_order_move = 'SPLIT_CASE'
                              AND NVL (so_line_rec (i).un_ex_flag, 'N') = 'Y'
                              AND NVL (so_line_rec (i).split_case, 'N') = 'Y'
                        THEN
                            lv_unschedule   := 'Y';
                            lv_order_move   := 'UNSCHEDULE_MOVE';
                        END IF;                        ---end w.r.t CCR0009825
                        ...
                        THEN
                            ...
                            --IF pv_order_move = 'ATRISK_MOVE'  -- added w.r.t CCR0009825
                            IF lv_order_move = 'ATRISK_MOVE' -- added w.r.t CCR0009825
                            THEN
                            ...
                            write_log (
                                   'updating the staging table for line ID for schedule - '
                                || so_line_rec (i).line_id
                                || ' Batch '
                        ...
                        write_log ('Before Split: ' || l_return_status);

                        /****************************************************************************************
                         * If unscheduling is success, then try split and schedule for bulks
                         ****************************************************************************************/
                        IF    ((l_return_status = 'Z' OR l_return_status = 'S') AND so_line_rec (i).split_case = 'Y' AND so_line_rec (i).split_order = 'ALL')
                           OR ((l_return_status = 'Z' OR l_return_status = 'S') AND so_line_rec (i).split_case = 'Y' AND so_line_rec (i).bulk_order_flag = 'Y' AND so_line_rec (i).split_order = 'BULK')
                        THEN
                            write_log ('Inside Split: ' || l_return_status);
                            ...
                            THEN
                                ...
                                ELSE
                                    ...
                                    IF lv_order_move = 'UNSCHEDULE_MOVE' --w.r.t CCR0009825
                                    THEN
                                    ...
                                    THEN
                                        ...
                                        --Changes for schedule and reschedule  --START w.r.t CCR0009825

                                        IF lv_order_move <> 'UNSCHEDULE_MOVE'
                                        THEN
                                            /****************************************************************************************
                                             * Schedule the original line
                                             ****************************************************************************************/
                                            ...
                                            END IF;
                                        END IF;
                                    --Changes for schedule and reschedule  --END w.r.t CCR0009825
    ...
    PROCEDURE sorting_creteria (pv_brand VARCHAR2, pn_org_id NUMBER, pn_sf_move_days OUT NUMBER, pn_ar_move_days OUT NUMBER, pv_split_order OUT VARCHAR2, pv_sorting_criteria OUT VARCHAR2, pv_sorting_order OUT VARCHAR2, pv_scheduling_win OUT VARCHAR2, pv_cursor_sorting OUT VARCHAR2
                                , pn_scheduling_range OUT NUMBER, pv_sec_sorting_criteria OUT VARCHAR2, pv_sec_sorting_order OUT VARCHAR2)
    AS
        lv_cursor              VARCHAR2 (3000);
        lv_cursor_sorting      VARCHAR2 (3000);
        lv_sorting             VARCHAR2 (300);
        lv_sorting_order       VARCHAR2 (300);
        lv_sec_sorting         VARCHAR2 (300);
        lv_sec_sorting_order   VARCHAR2 (300);
    BEGIN
        SELECT attribute3, attribute4, attribute5,
          ...
          INTO pn_sf_move_days, pn_ar_move_days, pv_split_order, pv_sorting_criteria,
                              ...
                              pv_sec_sorting_criteria, pv_sec_sorting_order
        ...
        --start as part of CCR0009825
        IF pv_sec_sorting_criteria = 'Creation Date'
        THEN
            lv_sec_sorting   := ' , order_creation_date ';
        ELSIF pv_sec_sorting_criteria = 'Request Date'
        THEN
            lv_sec_sorting   := ' , request_date ';
        ELSIF pv_sec_sorting_criteria = 'Ordered Date'
        THEN
            lv_sec_sorting   := ' , Ordered_Date ';
        END IF;

        IF pv_sec_sorting_order = 'DESC_ORDER'
        THEN
            lv_sec_sorting_order   := ' desc ';
        ELSIF pv_sec_sorting_order = 'ASC_ORDER'
        THEN
            lv_sec_sorting_order   := ' asc ';
        END IF;

        --End as part of CCR0009825
        pv_sec_sorting_criteria   := lv_sec_sorting;             -- CCR0009825
        pv_sec_sorting_order      := lv_sec_sorting_order;       -- CCR0009825
        lv_cursor_sorting         := lv_sorting || lv_sorting_order; --Commented as part of CCR0009825
        -- lv_cursor_sorting := lv_sorting || lv_sorting_order || lv_sec_sorting || lv_sec_sorting_order ; --Added as part of CCR0009825

        pv_cursor_sorting         := lv_cursor_sorting;           --CCR0009825
    EXCEPTION
        WHEN OTHERS
        THEN
            pn_sf_move_days       := 0;
    ...
    AS
        ...
        lv_sec_sorting_criteria   VARCHAR2 (300); --added as part of CCR0009825
        lv_sec_sorting_order      VARCHAR2 (300); --added as part of CCR0009825
    ...
    BEGIN
        ...
        sorting_creteria (
            ...
            pn_sf_move_days           => ln_sf_move_days,
            pn_ar_move_days           => ln_ar_move_days,
            pv_split_order            => lv_split_order,
            pv_sorting_criteria       => lv_sorting_criteria,
            pv_sorting_order          => lv_sorting_order,
            pv_scheduling_win         => lv_scheduling_win,
            pv_cursor_sorting         => lv_cursor_sorting,
            pn_scheduling_range       => ln_scheduling_range,
            pv_sec_sorting_criteria   => lv_sec_sorting_criteria, --added as part of CCR0009825
            pv_sec_sorting_order      => lv_sec_sorting_order); --added as part of CCR0009825
        ...
        THEN
            UPDATE xxdo.xxd_ont_auto_atp_levl_ordrs_t
               SET sf_el_flag = 'Y', --  order_split_type = lv_split_order  --commented as part of CCR0009825
                                     order_split_type = NVL (order_split_type, lv_split_order) --added as part of CCR0009825
        ...
        THEN
            ...
            UPDATE xxdo.xxd_ont_auto_atp_levl_ordrs_t
               SET sf_ex_flag = 'Y', --  new_lad = cancel_date - NVL (safe_move_days, ln_sf_move_days)  --w.r.t 1.3
                                     new_lad = GREATEST (cancel_date, latest_acceptable_date) --w.r.t 1.3
             WHERE     1 = 1
                   AND schedule_ship_date < atp_postive_date
                   AND cancel_date - NVL (safe_move_days, ln_sf_move_days) >
                       atp_postive_date
        ...
        THEN
            ...
            -- lv_cust_sorting := ' , NVL(attribute2,4) DESC '; --commented as part of CCR0009825
            lv_cust_sorting   := ' , NVL(attribute2,9999) DESC '; --added as part of CCR0009825

            IF lv_sorting_criteria = 'Request Date'
            THEN
                lv_cursor_sorting   :=
                       lv_cursor_sorting
                    || lv_sec_sorting_criteria
                    || lv_sec_sorting_order;     --added as part of CCR0009825
                lv_cursor   :=
    ...
    BEGIN
        ...
        sorting_creteria (
            ...
            pv_cursor_sorting         => lv_cursor_sorting,
            pn_scheduling_range       => ln_scheduling_range,
            pv_sec_sorting_criteria   => lv_sec_sorting_criteria, --added as part of
            pv_sec_sorting_order      => lv_sec_sorting_order); --added as part of
        ...
        THEN
            UPDATE xxdo.xxd_ont_auto_atp_levl_ordrs_t
               SET ar_el_flag = 'Y', new_lad = -- cancel_Date + NVL (atrisk_move_days, ln_ar_move_days), --w.r.t 1.3 CCR0010185
                                               GREATEST (cancel_Date + NVL (atrisk_move_days, ln_ar_move_days), latest_acceptable_date), -- order_split_type = lv_split_order  --commented as part of CCR0009825
                                                                                                                                         order_split_type = NVL (order_split_type, lv_split_order) --added as part of CCR0009825
             WHERE     1 = 1
                   AND schedule_ship_date < atp_postive_date
        ...
        THEN
            ...
            UPDATE xxdo.xxd_ont_auto_atp_levl_ordrs_t
               SET ar_ex_flag = ar_el_flag, -- new_lad =cancel_Date + NVL (atrisk_move_days, ln_ar_move_days) --w.r.t 1.3
                                            new_lad = GREATEST (cancel_Date + NVL (atrisk_move_days, ln_ar_move_days), latest_acceptable_date) --w.r.t 1.3
             WHERE     1 = 1
                   AND schedule_ship_date < atp_postive_date
                   AND cancel_Date + NVL (atrisk_move_days, ln_ar_move_days) >
                       atp_postive_date
        ...
        THEN
            ...
            THEN
                lv_cursor_sorting   :=
                       lv_cursor_sorting
                    || lv_sec_sorting_criteria
                    || lv_sec_sorting_order; --added as part of CCR0009825; --added as part of CCR0009825
                lv_cursor   :=
                    lv_cursor || lv_cursor_sorting || lv_cust_sorting;
            ELSE
                lv_cursor_sorting   :=
        ...
        THEN
            ...
            UPDATE xxdo.xxd_ont_auto_atp_levl_ordrs_t
               SET ar_ex_flag = ar_el_flag, --  new_lad = cancel_Date + NVL (atrisk_move_days, ln_ar_move_days)--w.r.t 1.3
                                            new_lad = GREATEST (cancel_Date + NVL (atrisk_move_days, ln_ar_move_days), latest_acceptable_date)
             WHERE     1 = 1
                   AND schedule_ship_date < atp_postive_date
                   AND cancel_Date + NVL (atrisk_move_days, ln_ar_move_days) >
                       atp_postive_date
                   AND batch_id = pn_batch_id
                   AND seq_number = pn_seq_number
                   AND inventory_item_id = pn_item_id              --w.r.t 1.3
                   AND ship_from_org_id = pn_organization_id
                   AND ar_el_flag = 'Y'
                   AND NVL (sf_el_flag, 'N') <> 'Y';
        END IF;
    ...
    BEGIN
        ...
        UPDATE xxdo.xxd_ont_auto_atp_levl_ordrs_t
           SET un_el_flag = 'Y', -- order_split_type = lv_split_order  --commented as part of CCR0009825
                                 order_split_type = NVL (order_split_type, lv_split_order) --added as part of CCR0009825
         WHERE     1 = 1
               AND batch_id = pn_batch_id
               AND inventory_item_id = pn_item_id
    ...
    -- This procedure also sends the email to recipients which are configured in lookup
    /************************************************************************************************/
            ...
            LOOP
                ...
                BEGIN
                    INSERT INTO XXDO.xxd_ont_auto_atp_levl_ordrs_t (
                                    ...
                                    safe_move_days,
                                    atrisk_move_days,
                                    order_split_type,   --added as part of CCR
                                    bulk_identifier,
                                    override_atp_flag,
                                    order_quantity_uom,
                                    sf_el_flag,
                        ...
                        SELECT ln_batch_id, rec_neg_atp_order_sku.seq_number, oola.inventory_item_id,
                               ...
                               flv.attribute4, flv.attribute5, --added as part CCR
                                                               oola.global_attribute19,
                               override_atp_date_code, order_quantity_uom, NULL,
                               NULL, NULL, NULL,
                               NULL, NULL, 'N',
                               gn_conc_request_id, SYSDATE, gn_user_id,
                               SYSDATE, gn_user_id
