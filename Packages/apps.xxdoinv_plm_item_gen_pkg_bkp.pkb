--
-- XXDOINV_PLM_ITEM_GEN_PKG_BKP  (Package Body) 
--
/* Formatted on 4/26/2023 4:37:12 PM (QP5 v5.362) */

CREATE OR REPLACE PACKAGE BODY APPS."XXDOINV_PLM_ITEM_GEN_PKG_BKP"
IS
/**********************************************************************************************************
 ...
 *********************************************************************/
   ...
   gv_style_intro_date                VARCHAR2 (200)     := NULL; --w.r.t Version 1.42
      ...
      BEGIN
         ...
         /*   AND category_set_id =
                          (SELECT category_set_id
                             FROM apps.mtl_category_sets
                            WHERE UPPER (category_set_name) = UPPER (pv_cat_set)); */                                                                                      -- Commented for 1.22.
                     ...
                     THEN
                        ...
                        THEN
                           ...
                           BEGIN
                              ...
                              validate_valueset ('SS' || rec_pros_cat.style,
                                                 'DO_STYLE_NUM',
                                                 rec_pros_cat.style,
                                                 gv_retcode,
                                                 gv_reterror,
                        ...
                        THEN
                           ...
                           BEGIN
                              ...
                              validate_valueset ('SL' || rec_pros_cat.style,
                                                 'DO_STYLE_NUM',
                                                 rec_pros_cat.style,
                                                 gv_retcode,
                                                 gv_reterror,
                           ...
                           BEGIN
                              ...
                              validate_valueset ('SR' || rec_pros_cat.style,
                                                 'DO_STYLE_NUM',
                                                 rec_pros_cat.style,
                                                 gv_retcode,
                                                 gv_reterror,
   ...
   BEGIN
      ...
      INSERT INTO xxdo.xxdo_plm_itemast_stg
                  (seq_num, parent_record_id,
                   ...
                   nrf_size_code, nrf_size_description
                  )
           VALUES (xxdo.xxdo_plm_itemast_stg_seq.NEXTVAL, pn_record_id,
                   ...
                   pv_nrf_size_code, pv_nrf_size_description
                  );
               ...
               SELECT show_inverse_con_rate
                ...
                WHERE conversion_date =
                         (SELECT MAX (conversion_date)
                            FROM gl_daily_rates_v
                           WHERE from_currency = lv_from_curr
...
--***********************************************************
      ...
      IF pv_tran_type = 'CREATE'
      THEN
      IF pn_orgn_id = gn_master_orgid --w.r.t 1.42 for Intro date
      THEN
       BEGIN
         lv_season_month := SUBSTR(gv_style_intro_date,1,instr(gv_style_intro_date,'/')-1);
         lv_season_year :=SUBSTR (pv_current_season,INSTR (pv_current_season, ' ') + 1,4);
         lv_season_name := SUBSTR (pv_current_season,1,INSTR (pv_current_season, ' ') -1);

         IF lv_season_name ='SPRING' AND lv_season_month in (10,11,12)
         THEN
      ...
      THEN
       ...
       IF pn_orgn_id = gn_master_orgid --w.r.t 1.42 for Intro date 
       THEN
      IF NVL(pv_intro_season,'XXXX') = NVL(pv_current_season,'YYYY')
      THEN
       BEGIN
         lv_season_month := SUBSTR(gv_style_intro_date,1,instr(gv_style_intro_date,'/')-1);
         lv_season_year :=SUBSTR (pv_current_season,INSTR (pv_current_season, ' ') + 1,4);
      ...
      THEN
         ...
         THEN
            ...
            THEN
               ...
               /*
                  BEGIN                                        --W.r.t Version 1.12
                     INSERT INTO xxdo.xxdo_plm_reprocess_stg
                                 (sequence_num,
                                  parent_record_id, rec_status, style, brand,
                                  uom, item_number, org_id,
                                  item_id, description, organization_code,
                                  current_season, life_cycle, template_id,
                                  request_id
                                 )
                          VALUES (xxdo.xxdo_plm_reprocess_seq.NEXTVAL,
                                  gn_record_id, 'N', pv_style, pv_brand,
                                  pv_primary_uom, pv_item_number, pn_orgn_id,
                                  pn_inv_item_id, pv_item_desc, pv_org_code,
                                  pv_current_season, NULL, ln_template_id,
                                  gn_conc_request_id
                                 );
                  ...
                  EXCEPTION
                     ...
                     THEN
                        fnd_file.put_line
                           (fnd_file.LOG,
                               'Error in Inserting into Items xxdo_plm_reprocess_stg Table :: '
                            || ' for the Item :: '
                            || pv_item_number
                            || ' for the Org :: '
                            || pn_orgn_id
   ...
   IS
      ...
      IS
         SELECT   stg.*,
                  ...
                  UPPER (   SUBSTR (stg.attribute1, 0, 4)
...
--***************************************************
         BEGIN
            ...
            SELECT list_header_id
             ...
             WHERE NAME = fnd_profile.VALUE ('XXDO_WHOLESALE_PRICELIST')
...
--***************************************************
         BEGIN
            ...
            SELECT list_header_id
              ...
              FROM apps.qp_list_headers_tl
             WHERE NAME = fnd_profile.VALUE ('XXDO_RETAIL_PRICELIST')
               --NAME = 'Retail - US'
               AND LANGUAGE = 'US';
              /*
...
--**************************************
            ...
            ELSIF     UPPER (rec_plm_data.colorway_state) = 'PRODUCTION'
                  --W.r.t Version 1.13
         ...
         LOOP
            ...
            BEGIN
               SELECT inventory_item_id, inventory_item_status_code,
                      attribute16, attribute24      --Corrected to intro season as part of CCR0006392
                 INTO ln_item_id, lv_exist_item_status,
                      lv_intro_season, lv_intro_date
                 FROM mtl_system_items_b
            ...
            BEGIN
               ...
               (   pv_item_number               => rec_process_records.item_number,
                   ...
                   pv_intro_season              => lv_intro_season, --w.r.t Version 1.42
                   pv_intro_date                => lv_intro_date, --w.r.t Version 1.42
               ...
               BEGIN
                  SELECT COUNT (*)
                   ...
                   WHERE style = rec_sourcing_records.style
                     ...
                     AND supplier_site_code = rec_sourcing_records.factory
                     AND SOURCE = 'PLM'
...
--********************************************
   ...
   END control_proc;
END XXDOINV_PLM_ITEM_GEN_PKG_BKP;
/
