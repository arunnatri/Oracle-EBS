--
-- XXD_XXDOAR037_CA_WRAP_PKG  (Package Body) 
--
/* Formatted on 4/26/2023 4:26:37 PM (QP5 v5.362) */
CREATE OR REPLACE PACKAGE BODY APPS."XXD_XXDOAR037_CA_WRAP_PKG"
AS
    /************************************************************************************************
    ...
    * 21-NOV-2016   Madhav Dhurjaty     v3.0     Changes for Canada 3PL Project
    ************************************************************************************************/
    ...
    PROCEDURE submit_request_layout (
        errbuf                    OUT VARCHAR2,
        retcode                   OUT NUMBER,
        p_org_id               IN     NUMBER,
        ...
        p_dir_loc              IN     VARCHAR2,
        p_batch_size           IN     NUMBER DEFAULT 300 -- Added by Infosys team. 12-May-2016.
                                                        )
    AS
        ...
        CURSOR csr_fetch_invoices IS
            SELECT details.customer_trx_id transaction_id
              FROM (  SELECT rct.customer_trx_id
                             ...
                             rct.reason_code
                                 reason_code,
                             xxdo_ar_invoice_print_ca.factored_flag (
                                 rct.customer_trx_id)
                                 factor_flag,
                             xxdo_ar_invoice_print_ca.remit_to_address_id (
                                 rct.customer_trx_id)
                                 remit_to_address_id,
                             ...
                             bill.country_name
                                 bill_country_name,
                             xxdo_ar_invoice_print_ca.address_dsp (
                                 bill.address1,
                                 bill.address2,
                                 bill.address3,
                                 bill.address4,
                             ...
                             term.description
                                 term_description,
                             xxdo_ar_invoice_print_ca.discount_amount_explanation (
                       ...
                       WHERE     rct.bill_to_customer_id = bill.customer_id
                             ...
                             AND apps.xxdo_ar_invoice_print_ca.remit_to_address_id (
                                     rct.customer_trx_id) =
                                 remit.address_id(+)
        ...
        CURSOR csr_batch_ids IS
              SELECT DISTINCT batch_id
                FROM xxd_xxdoar037_ca_invdtl_stg
    ...
    BEGIN
        ...
        BEGIN
            SELECT XXD_XXDOAR037_BATCHID_CA_SEQ.NEXTVAL
              INTO ln_batch_id
              FROM DUAL;
        EXCEPTION
            WHEN OTHERS
        ...
        LOOP
            ...
            INSERT INTO xxd_xxdoar037_ca_invdtl_stg
        ...
        LOOP
            ...
            ELSE
                ...
                lc_boolean2   :=
                    fnd_request.add_layout (
                        template_appl_name   => 'XXDO',
                        template_code        => 'XXDOAR037_CA_INV',
                        template_language    => 'en',
                        --Use language from template definition
                        template_territory   => 'US',
                        --Use territory from template definition
            ...
            ln_request_id   :=
                fnd_request.submit_request (
                    'XXDO',                                     -- application
                    'XXDOAR037_CA',                      -- program short name
                    'Transaction PDF File Generation  Deckers Canada',
                    -- description
                    SYSDATE,                                     -- start time
                    FALSE,                                      -- sub request
                    p_org_id,
            ...
            THEN
                ...
                ELSE
                    ...
                    IF     UPPER (l_chr_phase) = 'COMPLETED'
                       AND UPPER (l_chr_status) = 'NORMAL'
                    THEN
                        xxdo_ar_invoice_print_ca.update_pdf_generated_flag (
