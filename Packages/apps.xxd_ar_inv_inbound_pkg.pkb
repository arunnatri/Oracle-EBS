--
-- XXD_AR_INV_INBOUND_PKG  (Package Body) 
--
/* Formatted on 4/26/2023 4:31:24 PM (QP5 v5.362) */
CREATE OR REPLACE PACKAGE BODY APPS."XXD_AR_INV_INBOUND_PKG"
/***************************************************************************************
* Program Name : XXDO_AR_INV_INBOUND_PKG                                               *
* Language     : PL/SQL                                                                *
...
* WHO          :       WHAT      Desc                                    WHEN          *
* -------------- ----------------------------------------------------------------------*
* Kishan Reddy         1.0       Initial Version                         12-MAY-2022   *
* Kishan Reddy         1.1       CCR0010411: Added email logic for       17-JAN-2023   *
*                                AP Cross Border                                       *
* -------------------------------------------------------------------------------------*/
AS
    ...
    ************************************************************************************************/
    ...
    --
    PROCEDURE generate_ap_report_prc
    IS
    ...
    BEGIN
        ...
        ELSE
            ...
            -- fnd_file.put_line(fnd_file.log, 'Email sent to : ' || gv_def_mail_recips );
    ...
    BEGIN
        ...
        ELSE
            ...
            --   fnd_file.put_line(fnd_file.log, 'Email sent to : ' || gv_def_mail_recips );
            apps.do_mail_utils.send_mail_header ('erp@deckers.com', gv_def_mail_recips, 'Deckers AR Invoice Inbound Report ' || ' Email generated from ' || lv_inst_name || ' instance'
                                                 , ln_ret_val);
            ...
            do_mail_utils.send_mail_line ('Hello Team', ln_ret_val);
            do_mail_utils.send_mail_line (
                'Please see attached Deckers AR Invoice Inbound success/failed/error/reject Report.',
                ln_ret_val);
            do_mail_utils.send_mail_line ('', ln_ret_val);
            do_mail_utils.send_mail_line (
                'Note: This is auto generated mail, please donot reply.',
            ...
            do_mail_utils.send_mail_line ('Content-Type: text/xls',
                                          ln_ret_val);
            do_mail_utils.send_mail_line (
                   'Content-Disposition: attachment; filename="Deckers_AR_Inv_Report'
                || TO_CHAR (SYSDATE, 'RRRRMMDD_HH24MISS')
                || '.xls"',
                ln_ret_val);
    ...
    BEGIN
        ...
        generate_ap_report_prc;
    END main;
END xxd_ar_inv_inbound_pkg;
/
