--
-- XXDO_INTERCOM_PRICING_PKG  (Package Body) 
--
/* Formatted on 4/26/2023 4:33:51 PM (QP5 v5.362) */
CREATE OR REPLACE PACKAGE BODY APPS."XXDO_INTERCOM_PRICING_PKG"
AS
    /*****************************************************************
    ...
    * 9/15/2015            BT Technology Team      CR# 95 (2.0)
    * 11/3/2015            BT Technology Team      Defect# 431
    * 11/18/2015 Defect 685 Added p_source Parameter to Functions calling MAIN and MIAN
    * 11/18/2015 Defect 713 Modified DUTY calculations
    * 3/8/2016              Intercompany pricing function - replaced the API call to
    *                       query the QP Tables    v1.1
    *****************************************************************/
    ...
    PROCEDURE SEND_EMAIL_REPORT (p_message IN VARCHAR2)
    IS
    ...
    BEGIN
        ...
        v_def_mail_recips   := get_email_recips ('XXDO_INTERCOM_PRICING');
        apps.do_mail_utils.send_mail_header (apps.fnd_profile.VALUE ('DO_DEF_ALERT_SENDER'), v_def_mail_recips, 'Intercompany Pricing Error -' || TO_CHAR (SYSDATE, 'MM/DD/YYYY HH24:MI')
                                             , l_ret_val);
        ...
        do_mail_utils.send_mail_line (p_message, l_ret_val);
