--
-- XXD_MTL_CI_XREF_IFACE_PKG  (Package Body) 
--
/* Formatted on 4/26/2023 4:29:20 PM (QP5 v5.362) */
CREATE OR REPLACE PACKAGE BODY APPS.XXD_MTL_CI_XREF_IFACE_PKG
AS
    /*******************************************************************************
    ...
    TYPE XXD_MTL_CI_INT_TAB IS TABLE OF XXD_MTL_CI_XREFS_INFACE_STG_T%ROWTYPE
    ...
    IS
        ...
        CURSOR lcu_extract_count IS
            SELECT COUNT (*)
              FROM XXD_MTL_CI_XREFS_INFACE_STG_T
        ...
        CURSOR lcu_cust_item_data IS
            SELECT /*+ FIRST_ROWS(10) */
                   *
              FROM XXD_MTL_CUSTOMER_ITEM_XREFS_V XACI
             WHERE     EXISTS
                           (SELECT 1
                   ...
                   AND NOT EXISTS
                           (SELECT 1
                              FROM INV.MTL_CUSTOMER_ITEM_XREFS mci
                             WHERE mci.INVENTORY_ITEM_ID =
    ...
    BEGIN
        ...
        EXECUTE IMMEDIATE 'TRUNCATE TABLE XXD_CONV.XXD_MTL_CI_XREFS_INFACE_STG_T';
        ...
        LOOP
            ...
            FORALL i IN 1 .. gtt_mtl_ci_int_tab.COUNT
                INSERT INTO XXD_MTL_CI_XREFS_INFACE_STG_T
    ...
    AS
        ...
        IS
            SELECT *
              FROM XXD_MTL_CI_XREFS_INFACE_STG_T cust
        ...
        LOOP
            ...
            THEN
                ...
                LOOP
                    ...
                    IF lt_cust_data (xc_custr_rec).INVENTORY_ITEM_ID IS NULL
                    THEN
                        lc_cust_item_valid_data   := gc_no_flag;
                        lc_error_msg              :=
                               lc_error_msg
                            || ' INVENTORY_ITEM_ID Can not be null for the customer item '
                    ...
                    ELSE
                        ...
                        EXCEPTION
                            ...
                            THEN
                                ...
                                lc_error_msg              :=
                                       lc_error_msg
                                    || ' INVENTORY_ITEM_ID Not Available in system for the customer item '
                                    || lt_cust_data (xc_custr_rec).CUSTOMER_ITEM_NUMBER
                                    || 'and Item  '
                                    || lt_cust_data (xc_custr_rec).INVENTORY_ITEM_ID;
                            WHEN OTHERS
                    ...
                    IF lt_cust_data (xc_custr_rec).CUSTOMER_ITEM_NUMBER
                           IS NULL
                    THEN
                        lc_cust_item_valid_data   := gc_no_flag;
                        lc_error_msg              :=
                               lc_error_msg
                            || ' CUSTOMER_ITEM_NUMBER Can not be null for the customer item '
                            || lt_cust_data (xc_custr_rec).CUSTOMER_ITEM_NUMBER
                            || 'and Item  '
                            || lt_cust_data (xc_custr_rec).INVENTORY_ITEM_ID;
                    ELSE
                        ...
                        EXCEPTION
                            ...
                            THEN
                                ...
                                lc_error_msg              :=
                                       lc_error_msg
                                    || ' CUSTOMER_ITEM_NUMBER Not Available in system for the customer item '
                                    || lt_cust_data (xc_custr_rec).CUSTOMER_ITEM_NUMBER
                                    || 'and Item  '
                                    || lt_cust_data (xc_custr_rec).INVENTORY_ITEM_ID;
                            WHEN OTHERS
                    ...
                    THEN
                        UPDATE XXD_MTL_CI_XREFS_INFACE_STG_T
                    ...
                    ELSE
                        UPDATE XXD_MTL_CI_XREFS_INFACE_STG_T
                           SET ERROR_EXPLANATION = NULL, RECORD_STATUS = gc_validate_status, CUSTOMER_NUMBER = lt_cust_data (xc_custr_rec).CUSTOMER_NUMBER || '-' || lc_brand
                         WHERE     CUSTOMER_ITEM_NUMBER =
                                   lt_cust_data (xc_custr_rec).CUSTOMER_ITEM_NUMBER
                               AND CUSTOMER_NUMBER =
                                   lt_cust_data (xc_custr_rec).CUSTOMER_NUMBER
    ...
    **********************************************************************************************/
    IS
        TYPE type_ci_val_t IS TABLE OF MTL_CI_XREFS_INTERFACE%ROWTYPE
            INDEX BY BINARY_INTEGER;
        ...
        CURSOR c_get_valid_rec IS
            SELECT *
              FROM XXD_MTL_CI_XREFS_INFACE_STG_T XGPI
    ...
    BEGIN
        ...
        LOOP
            ...
            lt_ci_val_type (ln_valid_rec_cnt).INVENTORY_ITEM_ID   :=
                rec_get_valid_rec.INVENTORY_ITEM_ID;
            lt_ci_val_type (ln_valid_rec_cnt).MASTER_ORGANIZATION_CODE   :=
                rec_get_valid_rec.MASTER_ORGANIZATION_CODE;
            lt_ci_val_type (ln_valid_rec_cnt).PREFERENCE_NUMBER   :=
                rec_get_valid_rec.PREFERENCE_NUMBER;
            lt_ci_val_type (ln_valid_rec_cnt).INACTIVE_FLAG   :=
                rec_get_valid_rec.INACTIVE_FLAG;
        END LOOP;

        -------------------------------------------------------------------
        -- do a bulk insert into the MTL_CI_XREFS_INTERFACE table for the batch
        ----------------------------------------------------------------
        ...
        -------------------------------------------------------------------
        --Update the records that have been transferred to MTL_CI_XREFS_INTERFACE
        --as PROCESSED in staging table
        -------------------------------------------------------------------

        UPDATE XXD_MTL_CI_XREFS_INFACE_STG_T XGPI
