--
-- XXDO_AR_INVOICE_PRINT  (Package Body) 
--
/* Formatted on 4/26/2023 4:34:18 PM (QP5 v5.362) */
CREATE OR REPLACE PACKAGE BODY APPS."XXDO_AR_INVOICE_PRINT"
AS
    /*******************************************************************************
    ...
    * Madhav Dhurjaty                2.3      Fix for CCR0006682           27-SEP-2017
    * -----------------------------------------------------------------------------*/
    ...
    BEGIN
        ...
        THEN
            l_query   :=
                   l_query
                || ' and rct.creation_date >= TO_DATE('''
                || p_creation_date_low
                --|| ''',''RRRR/MM/DD HH24:MI:SS'')'; --Commented for CCR0006682
                || ''',''DD-MON-YY'')';                 --Added for CCR0006682
        END IF;

        l_step            := l_step + 10;                                --140
        ...
        THEN
            l_query   :=
                ...
                --|| ''',''RRRR/MM/DD HH24:MI:SS'')+1'; --Commented for CCR0006682
                || ''',''DD-MON-YY'')+1';               --Added for CCR0006682
        END IF;

        ----Added for CCR0005936 --End
        l_step            := l_step + 10;                                --150
    ...
    BEGIN
        ...
        THEN
            l_query   :=
                   l_query
                || ' AND rct.customer_trx_id IN (SELECT transaction_id FROM XXD_XXDOAR037_INV_DTL_STG WHERE batch_id = '''
                || p_batch_id
                || ''') ';
        END IF;
