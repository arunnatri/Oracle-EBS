--
-- XXDO_ATP_RESCHEDULE_PKG  (Package Body) 
--
/* Formatted on 4/26/2023 4:34:15 PM (QP5 v5.362) */
CREATE OR REPLACE PACKAGE BODY APPS."XXDO_ATP_RESCHEDULE_PKG"
IS
    /*******************************************************************************
    * $Header$
    ...
    *******************************************************************************/
    ...
    /* PROCEDURE IMPORT_SO_DATA(ERRBUF   OUT VARCHAR2,
    ...
    /*    V_STATUS VARCHAR2(2);
      ...
      BEGIN
        BEGIN
          DELETE FROM XXDO_ONT_SCHEDULE_HIS_T WHERE OU_ID = P_ORG_ID;

          INSERT INTO XXDO_ONT_SCHEDULE_HIS_T
            SELECT XXDO.XXDO_ONT_SCHEDULE_HIS_T_S.NEXTVAL,
                   OOLA.ORG_ID,
                   HOU.NAME,
        ...
        EXCEPTION
          WHEN OTHERS THEN
            ...
            DBMS_OUTPUT.PUT_LINE('Import SO Schedule Error,the Error is:' ||
                                 SQLERRM);
        END;
      END IMPORT_SO_DATA;*/
    ...
    /*
      ...
      BEGIN
        ...
        EXCEPTION
          WHEN OTHERS THEN
            V_STATUS := 'E';
            V_ERROR  := 'Purge SO Schedule History Data Error' || SQLERRM;
            DBMS_OUTPUT.PUT_LINE('Purge SO Schedule History Data Error,the Error is:' ||
                                 SQLERRM);
        END;
    ...
    FUNCTION GET_ITEM_STYLE_NUM (P_INVENTORY_ITEM_ID   IN NUMBER,
                                 P_INV_ORG_ID          IN NUMBER)
        RETURN VARCHAR2
    IS
        /*=============================================================================+
        | PROCEDURE: GET_ITEM_STYLE_NUM
        |
        | DESCRIPTION: Get Item Style Num by Inventory_Item_Id and Inventory_Organization_Id
        |
        | PARAMETERS:
        |   IN:  P_INVENTORY_ITEM_ID,  P_INV_ORG_ID
        |
    ...
    BEGIN
        BEGIN
            SELECT MC.ATTRIBUTE7
              INTO V_ITEM_STYLE
              FROM MTL_ITEM_CATEGORIES_V MIC, MTL_CATEGORIES MC
             WHERE     MIC.INVENTORY_ITEM_ID = P_INVENTORY_ITEM_ID
                   AND MIC.ORGANIZATION_ID = P_INV_ORG_ID
