--
-- XXD_CST_DUTY_ELE_INB_TR_PKG  (Package Body) 
--
/* Formatted on 4/26/2023 4:31:00 PM (QP5 v5.362) */
CREATE OR REPLACE PACKAGE BODY APPS."XXD_CST_DUTY_ELE_INB_TR_PKG"
IS
      /******************************************************************************************
 ...
 ****************************************************************************/
    ...
    PROCEDURE validate_prc (pv_file_name VARCHAR2-- ,pv_duty_override VARCHAR2)
                                                 )
    IS
        CURSOR fecth_tr_duty_elements IS
              SELECT ROWID, style_number, style_description,
                     country_of_origin, hts_code, duty_rate,
                     additional_duty, applicable_uom, destination_country,
                     preferential_rate, pr_start_date, pr_end_date,
                     default_duty_rate, default_additional_duty, default_applicable_uom,
                     effective_start_date, effective_end_date, department,
                     data_source, other_vendor_sites, intro_season,
                     size_run, pd_assigned, collection,
                     tarrif_description, itemcategory, --            currency_code,
                                                       additional_field1,
                     additional_field2, additional_field3, additional_field4,
                     additional_field5, additional_field6, additional_field7,
                     additional_field8, additional_field9, additional_field10,
                     additional_field11, additional_field12, additional_field13,
                     additional_field14, additional_field15, additional_field16,
                     additional_field17, additional_field18, additional_field19,
                     additional_field20, region, country,
                     country_code, operating_unit, organization_code,
                     inventory_org_id, coo_precedence, coo_preference_flag,
                     active_flag, preferential_duty_flag, rec_status,
                     error_msg, request_id, filename
                ...
                BEGIN
                    ...
                    LOOP
                        ...
                        THEN
                            BEGIN
                                ...
                                SELECT territory_code
                                 ...
                                 WHERE     1 = 1
                                       AND territory_code =
                                           v_tb_rec (i).country_of_origin;

                                IF lv_country_of_origin <>
                                   v_tb_rec (i).country_of_origin
                                THEN
                                    v_tb_rec (i).rec_status   := 'E';
                                    v_tb_rec (i).error_msg    :=
                                           v_tb_rec (i).error_msg
                                        || 'Country Of Origin is not Valid-';
                                END IF;
                            EXCEPTION
                                WHEN OTHERS
                                THEN
                                    write_log_prc (
                                        'Country of Origin' || SQLERRM);
                                    v_tb_rec (i).rec_status   := 'E';
                                    v_tb_rec (i).error_msg    :=
                                           v_tb_rec (i).error_msg
                                        || 'Country Of Origin is not Valid-';
                        ...
                        THEN
                            ...
                            v_tb_rec (i).error_msg    :=
                                   v_tb_rec (i).error_msg
                                || 'Country of Origin Cannnot be Null-';
                        END IF;
                        ...
                        THEN
                            BEGIN
                                SELECT ffvl.description
                                 ...
                                 WHERE     fvs.flex_value_set_id =
                                       ...
                                       AND ffvl.flex_value =
                                           v_tb_rec (i).destination_country;
                            ...
                            EXCEPTION
                                ...
                                THEN
                                    write_log_prc (
                                           'TR Destination Country not available in Value Set - '
                                        || v_tb_rec (i).destination_country);
                            ...
                            BEGIN
                                ...
                                IF lv_destination_country <>
                                   v_tb_rec (i).destination_country
                                THEN
                                    v_tb_rec (i).rec_status   := 'E';
                                    v_tb_rec (i).error_msg    :=
                                           v_tb_rec (i).error_msg
                                        || 'Destination Country is not Valid-';
                                END IF;
                            EXCEPTION
                                WHEN OTHERS
                                THEN
                                    write_log_prc (
                                        'Destination Country' || SQLERRM);
                                    v_tb_rec (i).rec_status   := 'E';
                                    v_tb_rec (i).error_msg    :=
                                           v_tb_rec (i).error_msg
                                        || 'Destination Country is not Valid-';
                        ...
                        THEN
                            ...
                            v_tb_rec (i).error_msg    :=
                                   v_tb_rec (i).error_msg
                                || 'Destination Country Cannnot be Null-';
                        END IF;
                        ...
                        IF TO_DATE (v_tb_rec (i).effective_start_date,
                                    'YYYY-MM-DD') =
                        ...
                        IF TO_DATE (v_tb_rec (i).effective_end_date,
                                    'YYYY-MM-DD') =
                           TO_DATE ('1900-01-01', 'YYYY-MM-DD')
                        THEN
                        ...
                        THEN
                            ...
                            write_log_prc (
                                'Assigned SYSDATE since Effective Start Date is NULL or Effective Start Date is 1900-01-01');
                            write_log_prc (v_tb_rec (i).effective_start_date);
                        END IF;

                        IF NVL (
                               TO_DATE (v_tb_rec (i).effective_end_date,
                                        'YYYY-MM-DD'),
                               TRUNC (SYSDATE)) <
                           TO_DATE (v_tb_rec (i).effective_start_date,
                                    'YYYY-MM-DD')
                        THEN
                            v_tb_rec (i).rec_status   := 'E';
                            v_tb_rec (i).error_msg    :=
                                   v_tb_rec (i).error_msg
                                || 'Effective End Date Cannnot Lesser than Effective Start Date-';
                            write_log_prc (
                                'Effective End Date Cannnot Lesser than Effective Start Date');
                        END IF;
                        ...
                        THEN
                            ...
                            v_tb_rec (i).error_msg    :=
                                   v_tb_rec (i).error_msg
                                || 'Effective End Date Cannnot Lesser than Sysdate-';
                            write_log_prc (
                                'Effective End Date Cannnot Lesser than Sysdate');
                        END IF;
                        ...
                        THEN
                            IF TO_DATE (v_tb_rec (i).pr_start_date,
                                        'YYYY-MM-DD') =
                               TO_DATE ('1900-01-01', 'YYYY-MM-DD')
                            THEN
                            ...
                            IF TO_DATE (v_tb_rec (i).pr_end_date,
                                        'YYYY-MM-DD') =
                               TO_DATE ('1900-01-01', 'YYYY-MM-DD')
                            THEN
                            ...
                            THEN
                                v_tb_rec (i).pr_start_date   :=
                                    TO_CHAR (TRUNC (SYSDATE), 'YYYY-MM-DD');
                                write_log_prc (
                                    'Assigned SYSDATE since PR Start Date is NULL or PR Start Date is 1900-01-01');
                                write_log_prc (v_tb_rec (i).pr_start_date);
                            END IF;

                            IF NVL (
                                   TO_DATE (v_tb_rec (i).pr_end_date,
                                            'YYYY-MM-DD'),
                                   TRUNC (SYSDATE)) <
                               TO_DATE (v_tb_rec (i).pr_start_date,
                                        'YYYY-MM-DD')
                            THEN
                                v_tb_rec (i).rec_status   := 'E';
                                v_tb_rec (i).error_msg    :=
                                       v_tb_rec (i).error_msg
                                    || 'PR End Date Cannnot Lesser than PR Start Date-';
                                write_log_prc (
                                    'PR End Date Cannnot Lesser than PR Start Date');
                            END IF;
                            ...
                            THEN
                                ...
                                v_tb_rec (i).error_msg    :=
                                       v_tb_rec (i).error_msg
                                    || 'PR End Date Cannnot Lesser than Sysdate-';
                                write_log_prc (
                                    'PR End Date Cannnot Lesser than Sysdate');
                            END IF;
                        END IF;
                        ...
                        THEN
                            ...
                            EXCEPTION
                                ...
                                THEN
                                    ...
                                    v_tb_rec (i).error_msg    :=
                                           v_tb_rec (i).error_msg
                                        || 'Unable to derive Department from xxd_common_items_v';
                            END;
                        END IF;
                                                                                       /*BEGIN
...
END;*/
                    ...
                    FORALL i IN v_tb_rec.FIRST .. v_tb_rec.LAST
                      SAVE EXCEPTIONS
                        UPDATE xxdo.xxd_cst_duty_ele_inb_stg_tr_t
                           SET region = v_tb_rec (i).region, destination_country = v_tb_rec (i).destination_country, department = v_tb_rec (i).department--,country_code = v_tb_rec(i).country_code
                                                                                                                                                         ...
                                                                                                                                                         ,
                               effective_start_date = v_tb_rec (i).effective_start_date, effective_end_date = v_tb_rec (i).effective_end_date, pr_start_date = v_tb_rec (i).pr_start_date,
                               pr_end_date = v_tb_rec (i).pr_end_date, rec_status = v_tb_rec (i).rec_status, error_msg = v_tb_rec (i).error_msg
                         WHERE ROWID = v_tb_rec (i).ROWID;

                    write_log_prc (
                        SQL%ROWCOUNT || ' Derived Values Updated...');
                EXCEPTION
                    WHEN e_bulk_errors
                    THEN
        ...
        UPDATE xxdo.xxd_cst_duty_ele_inb_stg_tr_t
         ...
         WHERE     1 = 1
               AND (style_number, country_of_origin, destination_country) IN
                       (  SELECT style_number, country_of_origin, destination_country
                            FROM xxdo.xxd_cst_duty_ele_inb_stg_tr_t
                           WHERE     1 = 1
                                 --AND rec_status = 'N'
                                 AND request_id = gn_request_id
                                 AND filename = pv_file_name
                        GROUP BY style_number, country_of_origin, destination_country
                          HAVING COUNT (1) > 1)
               --AND rec_status = 'N'
               AND request_id = gn_request_id
               AND filename = pv_file_name;
               ...
               AND (style_number, country_of_origin, destination_country) IN
                       (SELECT style_number, country_of_origin, destination_country
                         ...
                         WHERE     1 = 1
                               ...
                               AND t1.country_of_origin =
                                   t2.country_of_origin
                               AND t1.destination_country =
                                   t2.destination_country)
               -- AND error_msg IS NULL
               -- AND request_id <> gn_request_id
               AND filename <> pv_file_name;
        ...
        write_log_prc (
               SQL%ROWCOUNT
            || ' Element records updated with Current Flag as N ');
        COMMIT;
        ...
        write_log_prc (
            SQL%ROWCOUNT || ' Element records updated with Flag as V ');
        COMMIT;
 ...
 ***************************************************************************/
    ...
    AS
        CURSOR c_cat_main IS
            SELECT *
              FROM (SELECT DISTINCT item_number, inventory_org_id organization_id, 'TARRIF CODE' category_set_name,
                                    tarrif_code segment1, country segment3, --DEFAULT_CATEGORY segment3,
                                                                            default_category segment4,
                                    inventory_item_id inventory_item_id, GROUP_ID
    ...
    BEGIN
        write_log_prc (
               'Procedure insert_cat_to_stg_prc Begins...'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
        ...
        EXCEPTION
            ...
            THEN
                write_log_prc (
                    'Error while deleting Tarrif code category records from table xxdo.xxd_cst_duty_ele_cat_stg_t');
        END;
        ...
        LOOP
            ...
            THEN
                write_log_prc (
                       'No Valid records are present in the xxdo.xxd_cst_duty_ele_upld_stg_t table  and SQLERRM'
                    || SQLERRM);
            ELSE
                FORALL i IN 1 .. lt_main_tab.COUNT
                    --Inserting to Staging Table xxdo.xxd_cst_duty_ele_cat_stg_t
                    INSERT INTO xxdo.xxd_cst_duty_ele_cat_stg_t (record_id, batch_number, record_status, item_number, organization_id, category_set_name, segment1, -- SEGMENT2 ,
                                                                                                                                                                    segment3, segment4, inventory_item_id, created_by, creation_date, last_updated_by, last_update_date, error_msg
                                                                 , GROUP_ID)
                         VALUES (xxdo.xxd_cst_duty_ele_cat_stg_t_s.NEXTVAL, NULL, 'N', lt_main_tab (i).item_number, lt_main_tab (i).organization_id, lt_main_tab (i).category_set_name, lt_main_tab (i).segment1, --lt_main_tab (i).SEGMENT2,
        ...
        COMMIT;
        write_log_prc (
               'Procedure insert_cat_to_stg_prc Ends....'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
    EXCEPTION
        WHEN OTHERS
        THEN
            write_log_prc (
                'Error in procedure insert_cat_to_stg_prc:' || SQLERRM);
            NULL;
 ...
 *********************************************************/
    FUNCTION duty_uom_conversion_fnc (pv_inv_item_id IN VARCHAR2, pv_inv_org_id IN VARCHAR2, pv_additional_duty IN VARCHAR2, pv_applicable_uom IN VARCHAR2, pv_preferential_rate IN VARCHAR2, pv_preferential_duty_flag IN VARCHAR2
                                      , pv_default_additional_duty IN VARCHAR2, pv_default_applicable_uom IN VARCHAR2)
        RETURN VARCHAR2
    IS
    ...
    BEGIN
        ...
        BEGIN
            SELECT weight_uom_code, TO_NUMBER (unit_weight)
              INTO lv_sku_weight_uom_code, lv_sku_unit_weight
        ...
        EXCEPTION
            ...
            THEN
                write_log_prc (
                       ' Retiveing UOM Values Failed for the Inv Item ID and  Inv Org ID :'
                    || pv_inv_item_id
                    || '-'
                    || pv_inv_org_id);
        ...
        THEN
            ...
            ELSE
                ...
                EXCEPTION
                    ...
                    THEN
                        write_log_prc (
                               ' Retiveing UOM Weight Failed From Lookup for the SKU UOM Code and TR UOM Code :'
                            || lv_sku_weight_uom_code
                            || '-'
                            || pv_applicable_uom);
            ...
            lv_sku_conv_unit_weight   :=
                ROUND (lv_conv_uom_value, 6) * lv_sku_unit_weight;

            lv_tot_addl_duty   :=
                ROUND (lv_sku_conv_unit_weight, 6) * pv_additional_duty;

            -- write_log_prc ('Total Addtional Duty :'||ROUND (lv_tot_addl_duty,6));

            BEGIN
                lv_get_item_cost   :=
                    xxdoget_item_cost ('ITEMCOST', pv_inv_org_id, pv_inv_item_id
                                       , 'N');
            ...
            EXCEPTION
                ...
                THEN
                    write_log_prc (
                        'Exception occurred while calcluating the xxdoget_item_cost');
            END;
            ...
            THEN
                ...
                EXCEPTION
                    ...
                    THEN
                        lv_get_item_cost   := 0;
                        write_log_prc (
                            'Excpetion Occured while retiving the List Price');
                END;
            END IF;

            lv_tot_addl_duty_per   :=
                ROUND (lv_tot_addl_duty, 6) / lv_get_item_cost;

            write_log_prc (
                   'Total Addtional Duty Percentage :'
                || ROUND (lv_tot_addl_duty_per, 6));
        ...
        THEN
            ...
            lv_tot_addl_duty   :=
                  ROUND (lv_sku_conv_unit_weight, 6)
                * pv_default_additional_duty;

            write_log_prc (
                'Total Addtional Duty :' || ROUND (lv_tot_addl_duty, 6));
    ...
    EXCEPTION
        ...
        THEN
            apps.fnd_file.put_line (
                apps.fnd_file.LOG,
                'Exception Occured in Function duty_uom_conversion_fnc');
 ...
 ***************************************************************************/
    ...
    IS
        CURSOR cur_style_data IS
            SELECT DISTINCT style_number, country_of_origin, destination_country
              FROM xxdo.xxd_cst_duty_ele_inb_stg_tr_t s
             WHERE     1 = 1
                   AND rec_status = 'E'
                   AND active_flag = 'Y'
                   AND error_msg IN
                           ('Style Number is not Valid-', 'Unable To Derive OH Values From Value Set', 'Duty Element cannot be NULL')
                   AND request_id <> gn_request_id;

        CURSOR cur_tr_data IS
              SELECT style_number, style_description, country_of_origin,
                     hts_code, duty_rate, SUBSTR (additional_duty, 1, INSTR (additional_duty, ' ', 1) - 1) additional_duty,
                     applicable_uom, destination_country, preferential_rate,
                     pr_start_date, pr_end_date, default_duty_rate,
                     SUBSTR (default_additional_duty, 1, INSTR (default_additional_duty, ' ', 1) - 1) default_additional_duty, default_applicable_uom, effective_start_date,
                     effective_end_date, department, data_source,
                     other_vendor_sites, intro_season, size_run,
                     pd_assigned, collection, tarrif_description,
                     itemcategory-- ,region
                                 -- ,country
                                 -- ,country_code
                                 -- ,operating_unit
                                 ...
                                 -- ,coo_precedence
                                 -- ,coo_preference_flag
                                 , rec_status, error_msg,
                     active_flag, preferential_duty_flag, filename
                FROM xxdo.xxd_cst_duty_ele_inb_stg_tr_t s
               WHERE     1 = 1
                     AND rec_status = 'V'
                     AND active_flag = 'Y'
        ...
        IS
            SELECT parent_flex_value_low region,
                   ...
                   CASE
                       WHEN NVL (attribute3, 'X2X') = pv_country_of_origin
                       THEN
                           1
                       WHEN NVL (attribute4, 'X2X') = pv_country_of_origin
                       THEN
                           2
                       WHEN NVL (attribute5, 'X2X') = pv_country_of_origin
                       THEN
                           3
                       WHEN NVL (attribute6, 'X2X') = pv_country_of_origin
                       THEN
                           4
                       WHEN NVL (attribute7, 'X2X') = pv_country_of_origin
                       THEN
                           5
                       --BEGIN:Added for CCR0010051
                       WHEN NVL (attribute8, 'X2X') = pv_country_of_origin
                       THEN
                           6
                       WHEN NVL (attribute9, 'X2X') = pv_country_of_origin
                       THEN
                           7
                       WHEN NVL (attribute10, 'X2X') = pv_country_of_origin
                       THEN
                           8
                       WHEN NVL (attribute11, 'X2X') = pv_country_of_origin
                       THEN
                           9
                       WHEN NVL (attribute12, 'X2X') = pv_country_of_origin
                       THEN
                           10
                       WHEN NVL (attribute13, 'X2X') = pv_country_of_origin
                       THEN
                           11
                       WHEN NVL (attribute14, 'X2X') = pv_country_of_origin
                       THEN
                           12
                       WHEN NVL (attribute15, 'X2X') = pv_country_of_origin
                       THEN
                           13
                       WHEN NVL (attribute16, 'X2X') = pv_country_of_origin
                       THEN
                           14
                       WHEN NVL (attribute17, 'X2X') = pv_country_of_origin
                       THEN
                           15
            ...
            SELECT DISTINCT                       -- oh_val.attribute1 region,
             ...
             WHERE     1 = 1
                   ...
                   AND NVL (TRUNC (oh_val.start_date_active),
                            TRUNC (SYSDATE)) <=
                       TRUNC (SYSDATE)
                   AND NVL (TRUNC (oh_val.end_date_active), TRUNC (SYSDATE)) >=
                       TRUNC (SYSDATE)
        ...
        CURSOR cur_comm_items (pv_style_number VARCHAR2, pv_brand VARCHAR2, pv_inv_org_id VARCHAR2
                               , pv_department VARCHAR2)
        IS
            SELECT inventory_item_id inv_item_id, organization_id org_id, item_number item_number,
                   style_number style_number, color_code style_color, item_size item_size
              FROM apps.xxd_common_items_v
             WHERE     1 = 1
    ...
    BEGIN
        write_log_prc (
               'Procedure insert_into_custom_table_prc Begins...'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
            ...
            LOOP
                BEGIN
                    ...
                    THEN
                        BEGIN
                            UPDATE xxdo.xxd_cst_duty_ele_inb_stg_tr_t
                               SET rec_status = 'V', error_msg = '', request_id = gn_request_id
                        ...
                        EXCEPTION
                            ...
                            THEN
                                write_log_prc (
                                       'Exception Occurred while updating the rec status to V for the style number-'
                                    || i.style_number);
                        END;
                    END IF;
                    ...
                    BEGIN
                        UPDATE xxdo.xxd_cst_duty_ele_inb_stg_tr_t
                         ...
                         WHERE     style_number = i.style_number
                               ...
                               AND active_flag = 'Y'
                               AND error_msg IN
                                       ('Unable To Derive OH Values From Value Set', 'Duty Element cannot be NULL')
                               AND request_id <> gn_request_id;
                    EXCEPTION
                        WHEN OTHERS
                        THEN
                            write_log_prc (
                                   'Exception Occurred while updating the rec status to V for OH Values-'
                                || i.style_number);
                    END;
                EXCEPTION
                    WHEN OTHERS
                    THEN
                        ...
                        BEGIN
                            UPDATE xxdo.xxd_cst_duty_ele_inb_stg_tr_t
                               SET rec_status = 'E', error_msg = 'Style Number is not Valid-', request_id = gn_request_id
                             WHERE     style_number = i.style_number
                                   AND country_of_origin =
                                       i.country_of_origin
                                   AND destination_country =
                        ...
                        EXCEPTION
                            ...
                            THEN
                                write_log_prc (
                                       'Exception Occurred while updating the rec status E for the style number-'
                                    || i.style_number);
                        END;
                END;
            END LOOP;
        ...
        EXCEPTION
            ...
            THEN
                write_log_prc (
                    'Exception occurred while retriving the count from xxdo.xxd_cst_duty_ele_inb_stg_tr_t');
                ln_cnt   := 0;
        END;
        ...
        THEN
            ...
            LOOP
                write_log_prc (
                    ...
                    || i.default_additional_duty
                    || 'default_applicable_uom'
                    || i.default_applicable_uom
                    ...
                    || i.preferential_duty_flag);

                FOR l
                    IN cur_derive_regn_cnty_org (i.country_of_origin,
                                                 i.destination_country)
                LOOP
                    write_log_prc (
                           'Region:'
                    ...
                    BEGIN
                        ...
                        SELECT operating_unit, organization_code
                          INTO lv_operating_unit, lv_organization_code
                          FROM apps.org_organization_definitions
                         WHERE 1 = 1 AND organization_id = l.inventory_org_id;
                    ...
                    EXCEPTION
                        ...
                        THEN
                            write_log_prc (
                                   SQLERRM
                                || ' Failed to  Retrive Operating Unit for the Inv Org-'
                                || l.inventory_org_id);
                    END;

                    FOR j
                        IN cur_oh_ele_data (l.region,
                                            l.country_code,
                                            l.inventory_org_id)
                    LOOP
                        ...
                        EXCEPTION
                            ...
                            THEN
                                ...
                                EXCEPTION
                                    ...
                                    THEN
                                        ...
                                        EXCEPTION
                                                ...
                                                EXCEPTION
                                                        ...
                                                        EXCEPTION
                                                                ...
                                                                EXCEPTION
                                                                        ...
                                                                        EXCEPTION
                                                                                ...
                                                                                EXCEPTION
                                                                                        ...
                                                                                        EXCEPTION
                                                                                                ...
                                                                                                EXCEPTION
                                                                                                        ...
                                                                                                        EXCEPTION
                                                                                                                ...
                                                                                                                EXCEPTION
                                                                                                                        ...
                                                                                                                        EXCEPTION
                                                                                                                                ...
                                                                                                                                EXCEPTION
                                                                                                                                        ...
                                                                                                                                        EXCEPTION
                                                                                                                                                ...
                                                                                                                                                EXCEPTION
                                                                                                                                                    ...
                                                                                                                                                    THEN
                                                                                                                                                        ...
                                                                                                                                                        lv_oh_addl_duty   :=
                                                                                                                                                            NULL;
                                                                                                                                                        write_log_prc (
                                                                                                                                                               'OTHERS: No OHE For Country, Org, Department, Style Is Null'
                                                                                                                                                            || l.country_code
                                                                                                                                                            || '-'
                                                                                                                                                            || l.inventory_org_id
                                                                                                                                                            || '-'
                                                                                                                                            ...
                                                                                                                                            THEN
                                                                                                                                                ...
                                                                                                                                                lv_oh_addl_duty   :=
                                                                                                                                                    NULL;
                                                                                                                                                write_log_prc (
                                                                                                                                                       'OTHERS: No OHE For Country Not Null, Org, Department, Style Null'
                                                                                                                                                    || l.country_code
                                                                                                                                                    || '-'
                                                                                                                                                    || l.inventory_org_id
                                                                                                                                                    || '-'
                                                                                                                                    ...
                                                                                                                                    THEN
                                                                                                                                        ...
                                                                                                                                        lv_oh_addl_duty   :=
                                                                                                                                            NULL;
                                                                                                                                        write_log_prc (
                                                                                                                                               'OTHERS: No OHE For Country, Department, Style Is Null Org Not Null'
                                                                                                                                            || l.country_code
                                                                                                                                            || '-'
                                                                                                                                            || l.inventory_org_id
                                                                                                                                            || '-'
                                                                                                                            ...
                                                                                                                            THEN
                                                                                                                                ...
                                                                                                                                lv_oh_addl_duty   :=
                                                                                                                                    NULL;
                                                                                                                                write_log_prc (
                                                                                                                                       'OTHERS: No OHE For Country, Org Not Null, Department, Style Null'
                                                                                                                                    || l.country_code
                                                                                                                                    || '-'
                                                                                                                                    || l.inventory_org_id
                                                                                                                                    || '-'
                                                                                                                    ...
                                                                                                                    THEN
                                                                                                                        ...
                                                                                                                        lv_oh_addl_duty   :=
                                                                                                                            NULL;
                                                                                                                        write_log_prc (
                                                                                                                               'OTHERS: No OHE For Country, Style, Org Is Null Department Not Null'
                                                                                                                            || l.country_code
                                                                                                                            || '-'
                                                                                                                            || l.inventory_org_id
                                                                                                                            || '-'
                                                                                                            ...
                                                                                                            THEN
                                                                                                                ...
                                                                                                                lv_oh_addl_duty   :=
                                                                                                                    NULL;
                                                                                                                write_log_prc (
                                                                                                                       'OTHERS: No OHE For Country, Department Not Null, Org, Style Null'
                                                                                                                    || l.country_code
                                                                                                                    || '-'
                                                                                                                    || l.inventory_org_id
                                                                                                                    || '-'
                                                                                                    ...
                                                                                                    THEN
                                                                                                        ...
                                                                                                        lv_oh_addl_duty   :=
                                                                                                            NULL;
                                                                                                        write_log_prc (
                                                                                                               'OTHERS: No OHE For Country, Style Null, Org, Department Not Null'
                                                                                                            || l.country_code
                                                                                                            || '-'
                                                                                                            || l.inventory_org_id
                                                                                                            || '-'
                                                                                            ...
                                                                                            THEN
                                                                                                ...
                                                                                                lv_oh_addl_duty   :=
                                                                                                    NULL;
                                                                                                write_log_prc (
                                                                                                       'OTHERS: No OHE For Country, Org, Department, Not Null, Style Null'
                                                                                                    || l.country_code
                                                                                                    || '-'
                                                                                                    || l.inventory_org_id
                                                                                                    || '-'
                                                                                    ...
                                                                                    THEN
                                                                                        ...
                                                                                        lv_oh_addl_duty   :=
                                                                                            NULL;
                                                                                        write_log_prc (
                                                                                               'OTHERS: No OHE For Country, Org, Department Null, Style Not Null'
                                                                                            || l.country_code
                                                                                            || '-'
                                                                                            || l.inventory_org_id
                                                                                            || '-'
                                                                            ...
                                                                            THEN
                                                                                ...
                                                                                lv_oh_addl_duty   :=
                                                                                    NULL;
                                                                                write_log_prc (
                                                                                       'OTHERS: No OHE For Country, Style Not Null, Org, Department Null'
                                                                                    || l.country_code
                                                                                    || '-'
                                                                                    || l.inventory_org_id
                                                                                    || '-'
                                                                    ...
                                                                    THEN
                                                                        ...
                                                                        lv_oh_addl_duty   :=
                                                                            NULL;
                                                                        write_log_prc (
                                                                               'OTHERS: No OHE For Country, Department Null, Org, Style Not Null'
                                                                            || l.country_code
                                                                            || '-'
                                                                            || l.inventory_org_id
                                                                            || '-'
                                                            ...
                                                            THEN
                                                                ...
                                                                lv_oh_addl_duty   :=
                                                                    NULL;
                                                                write_log_prc (
                                                                       'OTHERS: No OHE For Country, Org, Style Not Null, Department Is Null'
                                                                    || l.country_code
                                                                    || '-'
                                                                    || l.inventory_org_id
                                                                    || '-'
                                                    ...
                                                    THEN
                                                        ...
                                                        lv_oh_addl_duty   :=
                                                            NULL;
                                                        write_log_prc (
                                                               'OTHERS: No OHE For Country, Org Null, Department, Style Not Null'
                                                            || l.country_code
                                                            || '-'
                                                            || l.inventory_org_id
                                                            || '-'
                                            ...
                                            THEN
                                                ...
                                                lv_oh_addl_duty      := NULL;
                                                write_log_prc (
                                                       'OTHERS: No OHE For Country, Department, Style Not Null, Org Null'
                                                    || l.country_code
                                                    || '-'
                                                    || l.inventory_org_id
                                                    || '-'
                                    ...
                                    THEN
                                        ...
                                        lv_oh_addl_duty      := NULL;
                                        write_log_prc (
                                               'OTHERS: No OHE For Country Null, Org, Department, Style Not Null'
                                            || l.country_code
                                            || '-'
                                            || l.inventory_org_id
                                            || '-'
                            ...
                            THEN
                                ...
                                lv_oh_addl_duty      := NULL;
                                write_log_prc (
                                       'OTHERS: No OHE For Country Org, Department, Style Not Null'
                                    || l.country_code
                                    || '-'
                                    || l.inventory_org_id
                                    || '-'
                        ...
                        FOR k IN cur_comm_items (i.style_number, lv_oh_brand, l.inventory_org_id
                                                 , i.department)
                        LOOP
                            ...
                            THEN
                                ...
                                END IF;
                            ELSIF     i.preferential_rate = 'Y'
                                  AND i.duty_rate IS NOT NULL
                                  AND lv_oh_duty IS NULL
                            THEN
                                lv_duty   := NULL;
                                lv_duty   := i.duty_rate;
                            ELSIF     i.preferential_rate = 'N'
                                  AND i.default_duty_rate IS NOT NULL
                                  AND lv_oh_duty IS NULL
                            THEN
                                lv_duty   := NULL;
                                lv_duty   := i.default_duty_rate;
                            ...
                            IF     i.duty_rate IS NULL
                               AND i.default_duty_rate IS NULL
                               AND lv_oh_duty IS NULL
                            THEN
                                -- End CCR0009885
                                lv_duty   := NULL;
                            ...
                            BEGIN
                                ...
                                INSERT INTO xxdo.xxd_cst_duty_ele_upld_stg_t (
                                                ...
                                                filename,
                                                additional_field1)
                                     VALUES (TRIM (l.region), TRIM (k.style_number), TRIM (k.style_color), TRIM (k.item_size), TRIM (lv_organization_code), TRIM (k.item_number), TRIM (i.country_of_origin), TRIM (i.destination_country), TRIM (UPPER (i.department)), TRIM (i.duty_rate), TRIM (i.additional_duty), TRIM (i.applicable_uom), TRIM (i.preferential_rate), TRIM (i.preferential_duty_flag), TRIM (i.pr_start_date), TRIM (i.pr_end_date), TRIM (i.default_duty_rate), TRIM (i.default_additional_duty), TRIM (i.default_applicable_uom), TRIM (i.effective_start_date), TRIM (i.effective_end_date), TRIM (lv_duty)--,'Y'             --Commented for CCR0010051
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    , 'N' --Added for CCR0010051
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         , NULL, NULL, TRIM (lv_oh_freight), TRIM (lv_oh_freight_duty), TRIM (lv_oh_oh_duty), TRIM (lv_oh_nonduty), NULL, TRIM (lv_oh_addl_duty), NULL -- i.hts_code
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      , TRIM (l.country), NULL, TRIM (lv_operating_unit), TRIM (l.inventory_org_id), TRIM (k.inv_item_id), TRIM (l.coo_precedence)--,NVL (TRIM (l.coo_preference_flag), 'N')
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  , 'N', TRIM (l_group_id)--,TRIM (pv_duty_override)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          , NULL, 'N', TRIM (i.active_flag), 'N', TRIM (i.error_msg), TRIM (gn_user_id), SYSDATE, SYSDATE, TRIM (gn_user_id), TRIM (gn_request_id), TRIM (i.filename)
                                             , TRIM (lv_oh_brand));
                            EXCEPTION
                                WHEN OTHERS
                                THEN
            ...
            BEGIN
                       ...
                       AND EXISTS
                               (SELECT 1
                                 ...
                                 WHERE     1 = 1
                                       ...
                                       AND s.country_of_origin =
                                           u.country_of_origin
                                       AND s.destination_country =
                ...
                write_log_prc (
                       SQL%ROWCOUNT
                    || ' Records Updated in Inbound Stg table as Processed');
                COMMIT;
            EXCEPTION
                WHEN OTHERS
                THEN
                    write_log_prc (
                           'Procesed: Updation Failed for Staging table: xxdo.xxd_cst_duty_ele_inb_stg_tr_t'
                        || SQLERRM);
            END;

            BEGIN
                UPDATE xxdo.xxd_cst_duty_ele_inb_stg_tr_t s
                   SET rec_status = 'E', error_msg = error_msg || 'Unable To Derive OH Values From Value Set'
                 WHERE     rec_status = 'V'
                       AND active_flag = 'Y'
                       AND NOT EXISTS
                               (SELECT 1
                ...
                write_log_prc (
                       SQL%ROWCOUNT
                    || ' Records Updated in Inbound Stg table as Errored');
                COMMIT;
            EXCEPTION
                WHEN OTHERS
                THEN
                    write_log_prc (
                           'Error: Updation Failed for Staging table: xxdo.xxd_cst_duty_ele_inb_stg_tr_t'
                        || SQLERRM);
            END;

            BEGIN
                UPDATE xxdo.xxd_cst_duty_ele_inb_stg_tr_t s
                   SET rec_status = 'E', error_msg = error_msg || 'Duty Element cannot be NULL'
                 WHERE     rec_status = 'V'
                       AND active_flag = 'Y'
                       AND EXISTS
                               (SELECT 1
                ...
                write_log_prc (
                       SQL%ROWCOUNT
                    || ' Records Updated in Inbound Stg table as Errored since Duty is NULL');
                COMMIT;
            EXCEPTION
                WHEN OTHERS
                THEN
                    write_log_prc (
                           'Duty: Updation Failed for Staging table: xxdo.xxd_cst_duty_ele_inb_stg_tr_t'
                        || SQLERRM);
            END;
            ...
            BEGIN
                ...
                UPDATE xxdo.xxd_cst_duty_ele_upld_stg_t
                   SET rec_status = 'X', error_msg = 'Duty Element cannot be NULL'
                 WHERE     rec_status = 'N'
                       AND error_msg IS NULL
                       AND active_flag = 'Y'
                       AND duty IS NULL
                ...
                write_log_prc (
                       SQL%ROWCOUNT
                    || ' Records Updated in SKU Stg table as Errored since Duty is NULL');
                COMMIT;
            EXCEPTION
                WHEN OTHERS
                THEN
                    write_log_prc (
                           'Duty: Updation Failed for Staging table: xxdo.xxd_cst_duty_ele_upld_stg_t'
                        || SQLERRM);
            END;
            ...
            BEGIN
                ...
                write_log_prc (
                       SQL%ROWCOUNT
                    || ' Element records updated with Active Flag as N ');
                COMMIT;
            EXCEPTION
                WHEN OTHERS
                THEN
                    write_log_prc (
                           ' Updation Failed for active flag in Staging table: xxdo.xxd_cst_duty_ele_upld_stg_t'
                        || SQLERRM);
            END;
            ...
            BEGIN
                UPDATE xxdo.xxd_cst_duty_ele_upld_stg_t u
                   SET coo_preference_flag   = 'Y'
                 WHERE     EXISTS
                               (SELECT s.style_number, s.region, s.country,
                                       s.inventory_org_id, s.coo_precedence
                                  FROM (  SELECT style_number style_number, region region, country country,
                                                 inventory_org_id inventory_org_id, MIN (coo_precedence) coo_precedence
                                            FROM xxdo.xxd_cst_duty_ele_upld_stg_t
                                           WHERE     coo_precedence <> 1000
                                                 AND request_id = gn_request_id
                                                 AND active_flag = 'Y'
                                        GROUP BY style_number, region, country,
                                                 inventory_org_id) s
                                 WHERE     s.style_number = u.style_number
                                       AND s.region = u.region
                                       AND s.country = u.country
                                       AND s.inventory_org_id =
                                           u.inventory_org_id
                                       AND s.coo_precedence =
                                           u.coo_precedence
                                       AND coo_precedence <> 1000
                ...
                write_log_prc (
                       SQL%ROWCOUNT
                    || ' Records updated COO preference flag as Y');
                COMMIT;
            EXCEPTION
                WHEN OTHERS
                THEN
                    write_log_prc (
                           ' COO preference flag Updation Failed for precedence <> 1000'
                        || SQLERRM);
            END;
...
WHERE (style_number,
  ...
  AND active_flag = 'Y';*/
                ...
                UPDATE xxdo.xxd_cst_duty_ele_upld_stg_t
                   SET coo_preference_flag   = 'Y'
                 WHERE     (style_number, country_of_origin, inventory_org_id) IN
                               (SELECT style_number, country_of_origin, inventory_org_id
                                  FROM (SELECT style_number, country_of_origin, inventory_org_id,
                                               ROW_NUMBER () OVER (PARTITION BY style_number, inventory_org_id ORDER BY style_number, inventory_org_id) rn
                                          FROM (  SELECT style_number, country_of_origin, inventory_org_id
                                                    FROM xxdo.xxd_cst_duty_ele_upld_stg_t t1
                                                   WHERE     coo_precedence =
                                                             1000
                                                         AND request_id =
                                                             gn_request_id
                                                         AND active_flag = 'Y'
                                                ...
                                                -- AND request_id = gn_request_id
                                                -- AND active_flag = 'Y')
                                                GROUP BY style_number, country_of_origin, inventory_org_id))
                                 WHERE rn = 1)
                       AND coo_preference_flag = 'N'
                       AND active_flag = 'Y'
                       AND request_id = gn_request_id;
                ...
                write_log_prc (
                       SQL%ROWCOUNT
                    || ' records updated with coo_preference_flag as Y ');
                COMMIT;
            EXCEPTION
                WHEN OTHERS
                THEN
            ...
            BEGIN
                ...
                write_log_prc (
                    'Primary Duty Flag Marked N:-' || SQL%ROWCOUNT);
                COMMIT;
                ...
                UPDATE xxdo.xxd_cst_duty_ele_upld_stg_t a
                   SET primary_duty_flag = 'Y', last_update_date = SYSDATE
                 WHERE     1 = 1
                       AND active_flag = 'Y'
                       AND a.coo_precedence != 1000
                       AND coo_precedence =
                           (SELECT MIN (coo_precedence)
                              FROM xxd_cst_duty_ele_upld_stg_t b
                             WHERE     1 = 1
                                   AND b.active_flag = 'Y'
                                   AND b.coo_precedence != 1000
                                   AND b.inventory_org_id =
                                       a.inventory_org_id
            ...
            EXCEPTION
                ...
                THEN
                    write_log_prc (
                        'Primary Duty Flag Updation Failed: ' || SQLERRM);
            END;
        --END: Added for CCR0010051
        ...
        THEN
            write_log_prc (
                   'No Valid records are present in the xxdo.xxd_cst_duty_ele_inb_stg_tr_t table and SQLERRM'
                || SQLERRM);
            apps.fnd_file.put_line (apps.fnd_file.LOG, '');
            apps.fnd_file.put_line (
                apps.fnd_file.LOG,
                'No Valid records are present in the xxdo.xxd_cst_duty_ele_inb_stg_tr_t table');
            apps.fnd_file.put_line (apps.fnd_file.LOG, '');
        END IF;
        ...
        write_log_prc (
               'Procedure insert_into_custom_table_prc Ends...'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
    EXCEPTION
        WHEN OTHERS
        THEN
 ...
 **************************************************************************/
    ...
    IS
        CURSOR exception_hdr IS
            SELECT 'Style Number' || gv_delim_pipe || 'Style Description' || gv_delim_pipe || 'Country Of Origin' || gv_delim_pipe || 'Hts Code' || gv_delim_pipe || 'Duty Rate' || gv_delim_pipe || 'Additional Duty' || gv_delim_pipe || 'Applicable UOM' || gv_delim_pipe || 'Destination Country' || gv_delim_pipe || 'Preferential Rate' || gv_delim_pipe || 'PR Start Date' || gv_delim_pipe || 'PR End Date' || gv_delim_pipe || 'Default Duty Rate' || gv_delim_pipe || 'Default Additional Duty' || gv_delim_pipe || 'Default Applicable UOM' || gv_delim_pipe || 'Effective Start Date' || gv_delim_pipe || 'Effective End Date' || gv_delim_pipe || 'Department' || gv_delim_pipe || 'Data Source' || gv_delim_pipe || 'Other Vendor Sites' || gv_delim_pipe || 'Intro Season' || gv_delim_pipe || 'Size Run' || gv_delim_pipe || 'Pd Assigned' || gv_delim_pipe || 'Collection' || gv_delim_pipe || 'Tarrif Description' || gv_delim_pipe || 'Item Category' || gv_delim_pipe || 'Additional Field1' || gv_delim_pipe || 'Additional Field2' || gv_delim_pipe || 'Additional Field3' || gv_delim_pipe || 'Additional Field4' || gv_delim_pipe || 'Additional Field5' || gv_delim_pipe || 'Additional Field6' || gv_delim_pipe || 'Additional Field7' || gv_delim_pipe || 'Additional Field8' || gv_delim_pipe || 'Additional Field9' || gv_delim_pipe || 'Additional Field10' || gv_delim_pipe || 'Additional Field11' || gv_delim_pipe || 'Additional Field12' || gv_delim_pipe || 'Additional Field13' || gv_delim_pipe || 'Additional Field14' || gv_delim_pipe || 'Additional Field15' || gv_delim_pipe || 'Additional Field16' || gv_delim_pipe || 'Additional Field17' || gv_delim_pipe || 'Additional Field18' || gv_delim_pipe || 'Additional Field19' || gv_delim_pipe || 'Additional Field20' || gv_delim_pipe || 'Current Flag' || gv_delim_pipe || 'Preferential Duty Flag' || gv_delim_pipe || 'Record Status' || gv_delim_pipe || 'Error Msg' || gv_delim_pipe || 'File Name' line_hdr
              FROM DUAL;

        CURSOR exception_rec IS
            SELECT TRIM (style_number) || gv_delim_pipe || TRIM (style_description) || gv_delim_pipe || TRIM (country_of_origin) || gv_delim_pipe || TRIM (hts_code) || gv_delim_pipe || TRIM (duty_rate) || gv_delim_pipe || TRIM (additional_duty) || gv_delim_pipe || TRIM (applicable_uom) || gv_delim_pipe || TRIM (destination_country) || gv_delim_pipe || TRIM (preferential_rate) || gv_delim_pipe || TRIM (pr_start_date) || gv_delim_pipe || TRIM (pr_end_date) || gv_delim_pipe || TRIM (default_duty_rate) || gv_delim_pipe || TRIM (default_additional_duty) || gv_delim_pipe || TRIM (default_applicable_uom) || gv_delim_pipe || TRIM (effective_start_date) || gv_delim_pipe || TRIM (effective_end_date) || gv_delim_pipe || TRIM (department) || gv_delim_pipe || TRIM (data_source) || gv_delim_pipe || TRIM (other_vendor_sites) || gv_delim_pipe || TRIM (intro_season) || gv_delim_pipe || TRIM (size_run) || gv_delim_pipe || TRIM (pd_assigned) || gv_delim_pipe || TRIM (collection) || gv_delim_pipe || TRIM (SUBSTR (tarrif_description, 1, 300)) || gv_delim_pipe || TRIM (itemcategory) || gv_delim_pipe || TRIM (additional_field1) || gv_delim_pipe || TRIM (additional_field2) || gv_delim_pipe || TRIM (additional_field3) || gv_delim_pipe || TRIM (additional_field4) || gv_delim_pipe || TRIM (additional_field5) || gv_delim_pipe || TRIM (additional_field6) || gv_delim_pipe || TRIM (additional_field7) || gv_delim_pipe || TRIM (additional_field8) || gv_delim_pipe || TRIM (additional_field9) || gv_delim_pipe || TRIM (additional_field10) || gv_delim_pipe || TRIM (additional_field11) || gv_delim_pipe || TRIM (additional_field12) || gv_delim_pipe || TRIM (additional_field13) || gv_delim_pipe || TRIM (additional_field14) || gv_delim_pipe || TRIM (additional_field15) || gv_delim_pipe || TRIM (additional_field16) || gv_delim_pipe || TRIM (additional_field17) || gv_delim_pipe || TRIM (additional_field18) || gv_delim_pipe || TRIM (additional_field19) || gv_delim_pipe || TRIM (additional_field20) || gv_delim_pipe || TRIM (preferential_duty_flag) || gv_delim_pipe || TRIM (active_flag) || gv_delim_pipe || TRIM (rec_status) || gv_delim_pipe || TRIM (error_msg) || gv_delim_pipe || TRIM (filename) line
              FROM (  SELECT style_number, style_description, country_of_origin,
                             hts_code, duty_rate, additional_duty,
                             applicable_uom, destination_country, preferential_rate,
                             pr_start_date, pr_end_date, default_duty_rate,
                             default_additional_duty, default_applicable_uom, effective_start_date,
                             effective_end_date, department, data_source,
                             other_vendor_sites, intro_season, size_run,
                             pd_assigned, collection, tarrif_description,
                             itemcategory, additional_field1, additional_field2,
                             additional_field3, additional_field4, additional_field5,
                             additional_field6, additional_field7, additional_field8,
                             additional_field9, additional_field10, additional_field11,
                             additional_field12, additional_field13, additional_field14,
                             additional_field15, additional_field16, additional_field17,
                             additional_field18, additional_field19, additional_field20,
                             preferential_duty_flag, active_flag, rec_status,
                             error_msg, filename
                        FROM xxdo.xxd_cst_duty_ele_inb_stg_tr_t
                       WHERE     1 = 1
                             AND rec_status = 'E'
    ...
    BEGIN
        ...
        lv_outbound_file   :=
               gn_request_id
            || '_Exception_RPT_'
            || TO_CHAR (SYSDATE, 'RRRR-MON-DD_HH24_MI_SS')
            || '.txt';                                     -- Added CCR0009885
        ...
        write_log_prc (
            'Exception Report ZIP File Name is - ' || lv_rep_file_name_zip);

        create_final_zip_prc (
            pv_directory_name   => 'XXD_CST_DUTY_ELE_EXC_DIR',
 ...
 ***************************************************************************/
    ...
    BEGIN
        ...
        BEGIN
               ...
               SET xcdeus.rec_status   = 'E',
                   error_msg          =
                       (SELECT           -- error_code||'-'||error_explanation
                         ...
                         WHERE     1 = 1
                               AND TO_NUMBER (xcdeus.inventory_item_id) =
                                   int.inventory_item_id
                               AND TO_NUMBER (xcdeus.inventory_org_id) =
                                   int.organization_id
            ...
            UPDATE xxdo.xxd_cst_duty_ele_upld_stg_t u
               SET rec_status = 'E', error_msg = 'Style Failed with Oracle'
             WHERE     (u.inventory_item_id, u.inventory_org_id) IN
                           (SELECT s.inventory_item_id, s.inventory_org_id
            ...
            write_log_prc (
                   SQL%ROWCOUNT
                || ' Records updated with the Rec Status as E for other Style Numbers');
            COMMIT;
        EXCEPTION
            WHEN OTHERS
            THEN
    ...
    IS
        CURSOR cur_data IS
            SELECT region, style_number, style_color,
                   organization_code, item_number, country_of_origin,
                   destination_country, duty, primary_duty_flag,
                   duty_start_date, duty_end_date, freight,
                   freight_duty, oh_duty, oh_nonduty,
                   factory_cost, addl_duty, tarrif_code,
                   country, default_category, operating_unit,
                   inventory_org_id, inventory_item_id, coo_preference_flag
              FROM xxdo.xxd_cst_duty_ele_upld_stg_t
             WHERE     1 = 1
    ...
    BEGIN
        write_log_prc (
               'Procedure insert_into_invval_duty_cost_prc Begins...'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
        ...
        LOOP
            FETCH cur_data BULK COLLECT INTO ele_data_tbl LIMIT g_limit;

            EXIT WHEN ele_data_tbl.COUNT = 0;

            write_log_prc (
                'Update the Duty Start Date, End Date if the combination of Inv Org Id, Inv Item Id, Country of Origin is already available in Duty Cost Table');

            BEGIN
                FORALL l_indx1 IN 1 .. ele_data_tbl.COUNT SAVE EXCEPTIONS
                    UPDATE xxdo.xxdo_invval_duty_cost xidc
                       SET last_update_date = SYSDATE, last_updated_by = gn_user_id, duty_start_date = NVL (duty_start_date, TO_DATE ('01-JAN-1952', 'DD-MON-YYYY')),
                           duty_end_date = NVL (TO_DATE (ele_data_tbl (l_indx1).duty_start_date, 'YYYY-MM-DD'), TRUNC (SYSDATE)) - 1
                     WHERE     inventory_org =
                               TO_NUMBER (
                                   ele_data_tbl (l_indx1).inventory_org_id)
                           AND inventory_item_id =
                               TO_NUMBER (
                                   ele_data_tbl (l_indx1).inventory_item_id)
                           AND NVL (country_of_origin, -1) =
                               NVL (ele_data_tbl (l_indx1).country_of_origin,
                                    -1)                 --Added for CCR0010051
                           AND NVL (
                                   duty_end_date,
                                   NVL (
                                       TO_DATE (
                                           ele_data_tbl (l_indx1).duty_start_date,
                                           'YYYY-MM-DD'),
                                       TRUNC (SYSDATE))) >=
                               NVL (
                                   TO_DATE (
                ...
                THEN
                    ...
                    LOOP
                        ...
                        write_log_prc (
                            ...
                            || ele_data_tbl (l_idx1).inventory_item_id
                            || ' for org - '
        ...
        LOOP
            ...
            write_log_prc (
                'Inserting Data into xxdo.xxdo_invval_duty_cost table');
            ...
            BEGIN
                FORALL l_indx IN 1 .. ele_data_tbl.COUNT SAVE EXCEPTIONS
                    INSERT INTO xxdo.xxdo_invval_duty_cost (
                                    ...
                                    creation_date,
                                    created_by)
                         VALUES (ele_data_tbl (l_indx).operating_unit, ele_data_tbl (l_indx).country_of_origin, ele_data_tbl (l_indx).primary_duty_flag, ele_data_tbl (l_indx).oh_duty, ele_data_tbl (l_indx).oh_nonduty, ele_data_tbl (l_indx).freight, ele_data_tbl (l_indx).freight_duty, ele_data_tbl (l_indx).addl_duty, ele_data_tbl (l_indx).inventory_org_id, ele_data_tbl (l_indx).inventory_item_id, ele_data_tbl (l_indx).duty, TO_DATE (ele_data_tbl (l_indx).duty_start_date, 'YYYY-MM-DD'), DECODE (ele_data_tbl (l_indx).coo_preference_flag, 'Y', NULL, TO_DATE (ele_data_tbl (l_indx).duty_end_date, 'YYYY-MM-DD')), ele_data_tbl (l_indx).style_color, --'N',
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         --NULL,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         --gn_request_id,
                ...
                THEN
                    ...
                    LOOP
                        ...
                        write_log_prc (
                            ...
                            || ele_data_tbl (l_idx).inventory_item_id
                            || ' for org - '
                            || ele_data_tbl (l_idx).inventory_org_id
                            || ' with error_code- '
                            || l_msg);
                    END LOOP;
        ...
        write_log_prc (
            ...
            || ' records into table xxdo_invval_duty_cost');
        write_log_prc (
               'Procedure insert_into_invval_duty_cost_prc Ends...'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
    EXCEPTION
        WHEN OTHERS
        THEN
            --retcode := 2;
            write_log_prc (
                   'Error in Procedure insert_into_invval_duty_cost_prc:'
                || SQLERRM);
 ...
 ***************************************************************************/
    ...
    IS
        CURSOR duty_ele_rep IS
              SELECT style_number, style_number || '-' || style_color style_color, item_size,
                     organization_code, item_number, country_of_origin,
                     duty, primary_duty_flag, duty_start_date,
                     duty_end_date, freight, freight_duty,
                     oh_duty, oh_nonduty, factory_cost,
                     addl_duty, tarrif_code, country,
                     default_category
                FROM xxdo.xxd_cst_duty_ele_upld_stg_t
               WHERE     1 = 1
                     ...
                     AND rec_status =
                         DECODE (NVL (pv_mode, 'Preview'),
                                 'Preview', 'N',
                                 'Process', 'P')
                     AND active_flag = 'Y'
                     AND request_id = gn_request_id
            ORDER BY DECODE (NVL (pv_dis_sku, 'N'), 'N', style_number), 'Y', style_number,
                     style_color, item_size;
 ...
 ***************************************************************************/
    ...
    BEGIN
        ...
        lv_rep_file_name_zip   :=
               SUBSTR (pv_rep_file_name,
                       1,
                       (INSTR (pv_rep_file_name, '.', -1) - 1))
            || '.zip';
        write_log_prc (
            'Duty Elements Report File Name is - ' || lv_rep_file_name);
        write_log_prc (
               'Duty Elements Report ZIP File Name is - '
            || lv_rep_file_name_zip);
        ...
        lv_rep_file_name_zip   :=
            lv_directory_path || lv_mail_delimiter || lv_rep_file_name_zip;
        ...
        lv_message1   :=
            ...
            || CHR (10)
            || CHR (10)
            || 'Please Find the Attached Deckers Costing Material Overhead Elements Report. '
            || CHR (10)
            || CHR (10)
            || lv_message2
            || CHR (10)
        ...
        xxdo_mail_pkg.send_mail (
            ...
            pv_subject        =>
                'Deckers Costing Material Overhead Elements Report',
 ...
 ***************************************************************************/
        ...
        CURSOR c_subelements IS
            SELECT ROWID, u.*
             ...
             WHERE     1 = 1
                   ...
                   AND u.inventory_org_id =
                       NVL (pv_inv_org, u.inventory_org_id)
    ...
    BEGIN
        ...
        apps.fnd_file.put_line (
            ...
            '                                                                               Deckers CM Update and Upload Cost Elements-Sku Level ');
        ...
        THEN
            ...
            UPDATE xxdo.xxd_cst_duty_ele_upld_stg_t
               SET rec_status = 'N', error_msg = NULL, request_id = gn_request_id,
                   created_by = gn_user_id, creation_date = SYSDATE, last_updated_by = gn_user_id,
            ...
            write_log_prc (
                SQL%ROWCOUNT || ' Error Records Updated with Flag N');
            COMMIT;
        ...
        THEN
            write_log_prc (
                'Update Request ID and WHO columns for unprocessed records in stg table...');

            UPDATE xxdo.xxd_cst_duty_ele_upld_stg_t
               SET request_id = gn_request_id, created_by = gn_user_id, creation_date = SYSDATE,
                   last_updated_by = gn_user_id, last_update_date = SYSDATE
             WHERE     rec_status = 'N'
                   AND error_msg IS NULL
                   AND active_flag = 'Y'
                   AND region = NVL (pv_region, region)
            ...
            write_log_prc (
                   SQL%ROWCOUNT
                || ' Records Updated with Request ID and WHO Columns');
            COMMIT;
        -- BEGIN
        -- SELECT COUNT(1)
        -- INTO ln_inb_cnt
        ...
        EXCEPTION
            ...
            THEN
                write_log_prc (
                    'Exception occurred while retriving the count from xxdo.xxd_cst_duty_ele_upld_stg_t');
                ln_cnt   := 0;
        END;
        ...
        apps.fnd_file.put_line (
            apps.fnd_file.output,
               ' Number of Rows Considered to Process Based on Parameters                    - '
            || ln_cnt);

        IF ln_cnt > 0
        THEN
            write_log_prc (
                   'Duty Calculation Process Begins...'
                || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
            ...
            LOOP
                ...
                THEN
                    ...
                    BEGIN
                        ...
                        LOOP
                            IF        v_tb_rec (i).preferential_rate = 'Y'
                                  AND v_tb_rec (i).duty_rate IS NOT NULL
                               OR     v_tb_rec (i).preferential_rate = 'N'
                                  AND v_tb_rec (i).default_duty_rate
                                          IS NOT NULL
                            THEN
                                IF    (v_tb_rec (i).preferential_rate = 'Y' AND v_tb_rec (i).additional_duty IS NOT NULL AND v_tb_rec (i).applicable_uom IS NOT NULL)
                                   OR (v_tb_rec (i).preferential_rate = 'N' AND v_tb_rec (i).default_additional_duty IS NOT NULL AND v_tb_rec (i).default_applicable_uom IS NOT NULL)
                                THEN
                                    BEGIN
                                        SELECT weight_uom_code, unit_weight
                                          INTO lv_sku_weight_uom_code, lv_sku_unit_weight
                                          FROM mtl_system_items_b
                                         WHERE     inventory_item_id =
                                    ...
                                    IF     lv_sku_weight_uom_code IS NOT NULL
                                       AND lv_sku_unit_weight IS NOT NULL
                                    THEN
                                        BEGIN
                                        ...
                                        EXCEPTION
                                            WHEN OTHERS
                                            THEN
                                                v_tb_rec (i).rec_status   :=
                                                    'E';
                                                v_tb_rec (i).error_msg   :=
                                                       v_tb_rec (i).error_msg
                                                    || 'Exception Occurred for calcluating Additional Duty Percentage-';
                                                write_log_prc (
                                                    ' Exception Occurred while calcluating Additional Duty Percentage');
                                                lv_tot_addl_duty_per   :=
                                                    NULL;
                                        END;
                                    ELSE
                                        ...
                                        v_tb_rec (i).error_msg    :=
                                               v_tb_rec (i).error_msg
                                            || 'WEIGHT_UOM_CODE OR UNIT_WEIGHT Not Available in Oracle-';
                                    END IF;
                                END IF;
                                ...
                                THEN
                                    v_tb_rec (i).duty   :=
                                          v_tb_rec (i).duty_rate
                                        + NVL (lv_tot_addl_duty_per, 0);
                                    v_tb_rec (i).duty_start_date   :=
                                        v_tb_rec (i).pr_start_date;
                                    v_tb_rec (i).duty_end_date   :=
                                        v_tb_rec (i).pr_end_date;
                                ELSIF v_tb_rec (i).preferential_rate = 'N'
                                THEN
                                    v_tb_rec (i).duty   :=
                                          v_tb_rec (i).default_duty_rate
                                        + NVL (lv_tot_addl_duty_per, 0);
                                    v_tb_rec (i).duty_start_date   :=
                                        v_tb_rec (i).effective_start_date;
                                    v_tb_rec (i).duty_end_date   :=
                    ...
                    EXCEPTION
                        ...
                        THEN
                            ...
                            write_log_prc (
                                   SQLERRM
                                || ' Other Error - Duty Validations Failed');
                    END;

                    write_log_prc (
                           'Duty Values Updated Process Begins...'
                        || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
                    ...
                    BEGIN
                        FORALL i IN v_tb_rec.FIRST .. v_tb_rec.LAST
                          SAVE EXCEPTIONS
                            UPDATE xxdo.xxd_cst_duty_ele_upld_stg_t
                               SET duty = v_tb_rec (i).duty, duty_start_date = v_tb_rec (i).duty_start_date, duty_end_date = v_tb_rec (i).duty_end_date,
                                   rec_status = v_tb_rec (i).rec_status, error_msg = v_tb_rec (i).error_msg
                             WHERE     rec_status = 'N'
                                   AND error_msg IS NULL
                                   AND active_flag = 'Y'
                                   AND request_id = gn_request_id
                        ...
                        write_log_prc (
                            SQL%ROWCOUNT || ' Duty Values Updated...');
                        COMMIT;
                    EXCEPTION
                        WHEN e_bulk_errors
                        THEN
                            ...
                            LOOP
                                ...
                                write_log_prc (
                                       'Failed to update Dute Values Element For Inventory Item- '
                    ...
                    write_log_prc (
                           'Duty Values Updated Process Ends...'
                        || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
                END IF;
            ...
            apps.fnd_file.put_line (
                apps.fnd_file.output,
                   ' Number of Rows Successfully Validated into Process Staging Table            - '
                || ln_ele_rec_success);
            apps.fnd_file.put_line (
                apps.fnd_file.output,
                   ' Number of Rows Errored in Process Staging Table Post Validation             - '
                || ln_ele_rec_error);
            apps.fnd_file.put_line (
                apps.fnd_file.output,
                '*************************************************************************************************');
            ...
            IF pv_mode = 'Preview'
            THEN
                write_duty_ele_report_prc (pv_region, pv_inv_org, pv_style,
                                           pv_color, pv_mode, pv_dis_sku,
                                           pv_send_mail, lv_rep_file_name);
            ...
            THEN
                write_log_prc (
                       'Procedure insert_into_interface_prc Process Begins...'
                    || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));

                write_log_prc (
                    'Truncating table CST_ITEM_CST_DTLS_INTERFACE.. ');
                    ...
                    THEN
                        ...
                        THEN
                            ...
                            EXCEPTION
                                ...
                                THEN
                                    write_log_prc (
                                        'Unable to Derive Duty Element from CICD');
                                    lv_duty   := NULL;
                            END;
                        ...
                        THEN
                            ...
                            INSERT INTO cst_item_cst_dtls_interface (
                                            ...
                                            GROUP_ID,
                                            attribute15)
                                 VALUES (i.inventory_item_id, i.inventory_org_id, gc_duty, -- i.duty
                                                                                           lv_duty, gn_cost_element_id, gc_cost_type, l_duty_basis, gn_process_flag, SYSDATE, gn_user_id, SYSDATE, gn_user_id
                                         , l_group_id, i.country_of_origin);
                        END IF;
                        ...
                        THEN
                            ...
                            INSERT INTO cst_item_cst_dtls_interface (
                                            ...
                                            GROUP_ID,
                                            attribute15)
                                 VALUES (i.inventory_item_id, i.inventory_org_id, gc_freight, -- i.freight,
                        ...
                        THEN
                            ...
                            INSERT INTO cst_item_cst_dtls_interface (
                                            ...
                                            GROUP_ID,
                                            attribute15)
                                 VALUES (i.inventory_item_id, i.inventory_org_id, gc_oh_duty, --i.oh_duty,
                        ...
                        THEN
                            ...
                            INSERT INTO cst_item_cst_dtls_interface (
                                            ...
                                            GROUP_ID,
                                            attribute15)
                                 VALUES (i.inventory_item_id, i.inventory_org_id, gc_oh_nonduty, --i.oh_nonduty,
                        ...
                        THEN
                            ...
                            INSERT INTO cst_item_cst_dtls_interface (
                                            ...
                                            GROUP_ID,
                                            attribute15)
                                 VALUES (i.inventory_item_id, i.inventory_org_id, gc_freight_du, -- i.freight_duty,
                        ...
                        THEN
                            ...
                            l_item_tbl_typ (api_index).transaction_type   :=
                                'UPDATE';
                ...
                BEGIN
                    UPDATE xxdo.xxd_cst_duty_ele_upld_stg_t xcdeus
                     ...
                     WHERE     EXISTS
                                   (SELECT 1
                                     ...
                                     WHERE     1 = 1
                                           AND TO_NUMBER (
                                                   xcdeus.inventory_item_id) =
                                               inventory_item_id
                                           AND TO_NUMBER (
                                                   xcdeus.inventory_org_id) =
                                               organization_id
                    ...
                    write_log_prc (
                           SQL%ROWCOUNT
                        || ' Records successfully Updated with status I for coo_preference_flag Y Records');
                ...
                BEGIN
                    ...
                    write_log_prc (
                           SQL%ROWCOUNT
                        || ' Records successfully Updated with status NA for coo_preference_flag N Records ');
                ...
                EXCEPTION
                    ...
                    THEN
                        write_log_prc (
                            'Exception Occured while updating status NA to staging table:');
                END;
                ...
                THEN
                    ...
                    EXCEPTION
                        ...
                        THEN
                            write_log_prc (
                                'Exception Occured while fetching v_processed:');
                            v_processed   := 'N';
                    END;
                    ...
                    THEN
                        ...
                        write_log_prc (
                            'submitting custom table upload program');
            ...
            THEN
                BEGIN
                    ...
                    ln_rec_fail_oracle   :=
                        ln_rec_oracle - ln_rec_success_oracle;
                ...
                EXCEPTION
                    ...
                    THEN
                        write_log_prc (
                            'Exception Occured while retriving the records status count');
                END;
                ...
                apps.fnd_file.put_line (
                    apps.fnd_file.output,
                       ' Number of Rows Successful in Interface table                                - '
                    || ln_rec_success_oracle);
                apps.fnd_file.put_line (
                    apps.fnd_file.output,
                       ' Number of Rows Errored in Interface table                                   - '
                    || ln_rec_fail_oracle);
                apps.fnd_file.put_line (
                    apps.fnd_file.output,
                    '*************************************************************************************************');
                apps.fnd_file.put_line (
                    apps.fnd_file.output,
                       ' Number of Rows Successful in Process Staging table After Cost Import        - '
                    || ln_rec_success);
                apps.fnd_file.put_line (
                    apps.fnd_file.output,
                       ' Number of Rows Not Considered in Process Staging table by coo preference    - '
                    || ln_rec_na);
                apps.fnd_file.put_line (
                    apps.fnd_file.output,
                       ' Total Errored Records in Process Staging Table After Cost Import            - '
                    || ln_rec_fail);
                apps.fnd_file.put_line (
                    apps.fnd_file.output,
                    '*************************************************************************************************');
            ...
            apps.fnd_file.put_line (
                apps.fnd_file.output,
                   ' Number of Rows Successfully Inserted into table xxdo_invval_duty_cost       - '
                || g_invval_cnt);
            apps.fnd_file.put_line (
                apps.fnd_file.output,
                '**********************************************************************************************');
        ...
        THEN
            ...
            apps.fnd_file.put_line (
                apps.fnd_file.output,
                'No Valid records are present in the xxdo.xxd_cst_duty_ele_upld_stg_t table');
        END IF;

        write_log_prc (
               'Procedure insert_into_interface_prc Ends...'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
    EXCEPTION
        WHEN user_exception
        THEN
 ...
 ***************************************************************************/
    ...
    AS
        ...
        IS
            SELECT structure_id, category_set_id
             ...
             WHERE 1 = 1 AND category_set_name = cp_category_set_name;
    ...
    BEGIN
        write_log_prc (
               'Procedure get_category_id_prc Begins....'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
        ...
        LOOP
            ...
            LOOP
                ...
                write_log_prc (
                    'l_segment_array-' || l_segment_array (c_segments.ROWNUM));
            ...
            write_log_prc (
                'l_n_segments:' || l_n_segments || ' l_delim:' || l_delim);
            ...
            write_log_prc (
                   'OPEN get_category_id_prc l_concat_segs  => '
                || l_concat_segs);
            ...
            write_log_prc (
                   'OPEN get_category_id_prc structure_id   => '
                || l_category_rec.structure_id);
            write_log_prc (
                   'OPEN get_category_id_prc l_concat_segs  => '
                || l_concat_segs);
...
p_message   =>    'Lenght of l_concat_segs  => '
            ...
            OPEN get_category_id_prc (l_category_rec.structure_id,
                                      l_concat_segs);
            ...
            write_log_prc (
                   'OPEN get_category_id_prc gn_category_id  => '
                || gn_category_id);
            ...
            THEN
                ...
                THEN
                    ...
                    xxd_common_utils.record_error (
                        ...
                        p_org_id       => gn_org_id,
                        p_program      =>
                            'Deckers Cost Elements Category Assignment Program',
            ...
            ELSE
                write_log_prc (
                    'OPEN get_category_id_prc l_success  => False');
        ...
        write_log_prc (
               'Procedure get_category_id_prc Ends....'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
    EXCEPTION
        WHEN NO_DATA_FOUND
        THEN
            write_log_prc (
                'Error in procedure get_category_id_prc:' || SQLERRM);
            l_messages   := SQLERRM;
            xxd_common_utils.record_error (
                p_module       => 'INV',
                p_org_id       => gn_org_id,
 ...
 ***************************************************************************/
    ...
    FUNCTION validate_valueset_value_fn (p_category_set_name IN VARCHAR2, p_application_column_name IN VARCHAR2, p_flex_value IN VARCHAR2
    ...
    BEGIN
        write_log_prc (
               'Procedure validate_valueset_value_fn Begins...'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
        write_log_prc (
               'validate_valueset_value_fn for '
        ...
        write_log_prc (
               'Procedure validate_valueset_value_fn Ends...'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
    EXCEPTION
        WHEN NO_DATA_FOUND
        THEN
            ...
            write_log_prc (
                'Error in function validate_valueset_value_fn:' || SQLERRM);
            RETURN 'E';
        WHEN OTHERS
        THEN
            gn_record_error_flag   := 1;
 ...
 ***************************************************************************/
    ...
    IS
        CURSOR cur_item_category IS
            SELECT *
              FROM xxdo.xxd_cst_duty_ele_cat_stg_t
             WHERE 1 = 1 AND record_status IN (gc_new_status); --,gc_error_status);
    ...
    BEGIN
        write_log_prc (
               'Procedure inv_category_validation_prc Begins...'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
        ...
        write_log_prc (
               'Procedure inv_category_validation_prc Ends...'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
    EXCEPTION
        WHEN OTHERS
        THEN
            write_log_prc (
                'Error in Procedure inv_category_validation_prc:' || SQLERRM);
            errbuf    := SQLERRM;
            retcode   := 2;
            lc_err_msg   :=
 ...
 ***************************************************************************/
    ...
    BEGIN
        write_log_prc (
               'Procedure create_category_assignment_prc Begins...'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
        ...
        write_log_prc (
               'Procedure create_category_assignment_prc Ends...'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
    EXCEPTION
        WHEN NO_DATA_FOUND
        THEN
            write_log_prc (
                   'Error in procedure create_category_assignment_prc:'
                || SQLERRM);
            l_messages   := SQLERRM;
            xxd_common_utils.record_error (
                p_module       => 'INV',
 ...
 ***************************************************************************/
    ...
    PROCEDURE update_category_assignment_prc (p_category_id mtl_categories_b.category_id%TYPE, p_old_category_id mtl_categories_b.category_id%TYPE, p_category_set_id mtl_category_sets_tl.category_set_id%TYPE
    ...
    BEGIN
        write_log_prc (
               'Procedure update_category_assignment_prc Begins...'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
        ...
        write_log_prc (
               'Procedure update_category_assignment_prc Ends...'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
    EXCEPTION
        WHEN NO_DATA_FOUND
        THEN
            write_log_prc (
                   'Error in procedure update_category_assignment_prc:'
                || SQLERRM);
            l_messages   := SQLERRM;
            ...
            xxd_common_utils.record_error (
                ...
                p_error_msg    =>
                    NVL (SUBSTR (l_messages, 2000),
                         'Error in update_category_assignment_prc'),
                p_created_by   => gn_user_id,
                p_request_id   => gn_conc_request_id,
                p_more_info1   => 'update_category_assignment_prc',
                p_more_info2   => gn_inventory_item,
 ...
 ***************************************************************************/
    ...
    BEGIN
        write_log_prc (
               'Procedure cat_assignment_child_program_prc Begins...'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
        ...
        retcode   := 0;
        write_log_prc (
               'Procedure cat_assignment_child_program_prc Ends...'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
    EXCEPTION
        WHEN OTHERS
        THEN
            write_log_prc (
                   'Error in procedure cat_assignment_child_program_prc:'
                || SQLERRM);
            retcode   := 2;
 ...
 ***************************************************************************/
    ...
    BEGIN
        write_log_prc (
               'Procedure inv_category_load_prc Begins...'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
        --gc_debug_flag := p_debug;

        write_log_prc (
            'Insert succesful records from SKU Table to Category Table');

        insert_cat_to_stg_prc (lv_errbuf, lv_retcode);

        BEGIN
            SELECT DISTINCT GROUP_ID
              INTO l_group_id
              FROM xxdo.xxd_cst_duty_ele_cat_stg_t
             WHERE 1 = 1 AND category_set_name = pv_category_set_name;
...
END LOOP;*/
            ...
            LOOP
                ...
                LOOP
                    ...
                    EXCEPTION
                        ...
                        THEN
                            write_log_prc (
                                ' Update Batch Number Exception' || SQLERRM);
                    END;
                       ...
                       (SELECT MIN (batch_number)
                         ...
                         WHERE     1 = 1
                               ...
                               AND NVL (x1.segment4, 'XX') =
                                   NVL (x2.segment4, 'XX')) --  WHERE CATEGORY_SET_NAME = 'TARRIF CODE'
                                                           ;
                ...
                THEN
                    ...
                    EXCEPTION
                        ...
                        THEN
                            ...
                            write_log_prc (
                                   'Calling WAIT FOR REQUEST XXD_CST_ELE_CAT_CHILD_PRG error'
                                || SQLERRM);
                        WHEN OTHERS
                        THEN
                            retcode   := 2;
            ...
            write_log_prc (
                'Calling WAIT FOR REQUEST XXD_CST_ELE_CAT_CHILD_PRG to complete');
            ...
            SELECT COUNT (*)
              ...
              FROM xxdo.xxd_cst_duty_ele_cat_stg_t
             WHERE 1 = 1 AND GROUP_ID = l_group_id AND record_status = 'P';
        ...
        write_log_prc (
               'Procedure inv_category_load_prc Ends...'
            || TO_CHAR (SYSDATE, 'dd-mon-yyyy hh:mi:ss'));
    EXCEPTION
        WHEN OTHERS
        THEN
            write_log_prc (
                'Error in procedure inv_category_load_prc:' || SQLERRM);
            errbuf    := SQLERRM;
            retcode   := 2;
            lc_err_msg   :=
                   'Unexpected error while cursor fetching into PL/SQL table - '
    ...
    ***********************************************************************************/
    ...
    BEGIN
        ...
        write_log_prc (
            'Delete XXD_CST_DUTY_ELE_INB_STG_TR_T Table Data Based on Purge Parameter:');
        write_log_prc (
            '-------------------------------------------------------------------------');
        ...
        BEGIN
            ...
            write_log_prc (
                   SQL%ROWCOUNT
                || ' Rows Deleted from XXDO.XXD_CST_DUTY_ELE_INB_STG_TR_T Table');
        EXCEPTION
            WHEN OTHERS
            THEN
                write_log_prc (
                    'Failed to Delete Records from XXDO.XXD_CST_DUTY_ELE_INB_STG_TR_T Table');
        END;
    ...
    ***********************************************************************************/
    ...
    BEGIN
        ...
        write_log_prc (
            'Delete xxd_cst_duty_ele_upld_stg_t Table Data Based on Purge Parameter:');
        write_log_prc (
            '-------------------------------------------------------------------------');
        ...
        BEGIN
            ...
            write_log_prc (
                   SQL%ROWCOUNT
                || ' Rows Deleted from xxdo.xxd_cst_duty_ele_upld_stg_t Table');
        EXCEPTION
            WHEN OTHERS
            THEN
                write_log_prc (
                    'Failed to Delete Records from xxdo.xxd_cst_duty_ele_upld_stg_t Table');
        END;
    ...
    ***********************************************************************************/
    ...
    BEGIN
        ...
        write_log_prc (
            'Delete xxd_cst_duty_ele_cat_stg_t Table Data Based on Purge Parameter:');
        write_log_prc (
            '-------------------------------------------------------------------------');
        ...
        BEGIN
            ...
            write_log_prc (
                   SQL%ROWCOUNT
                || ' Rows Deleted from xxdo.xxd_cst_duty_ele_cat_stg_t Table');
        EXCEPTION
            WHEN OTHERS
            THEN
                write_log_prc (
                    'Failed to Delete Records from xxdo.xxd_cst_duty_ele_cat_stg_t Table');
        END;
    ...
    ***************************************************************************/
    ...
    BEGIN
        ...
        write_log_prc (
            'Delete xxdo_invval_duty_cost Table Data Based on Purge Parameter:');
        write_log_prc (
            '-----------------------------------------------------------------');
        ...
        BEGIN
            ...
            SELECT COUNT (1) INTO ln_tot_cnt FROM xxdo.xxdo_invval_duty_cost;

            write_log_prc (
                'xxdo.xxdo_invval_duty_cost Total Count: ' || ln_tot_cnt);
        EXCEPTION
            WHEN OTHERS
            THEN
                write_log_prc (
                    'Failed to Retrive count of XXDO.XXDO_INVVAL_DUTY_COST');
        END;
        ...
        BEGIN
            INSERT INTO xxdo.xxdo_invval_duty_cost_bkp (operating_unit, country_of_origin, primary_duty_flag, oh_duty, oh_nonduty, additional_duty, inventory_org, inventory_item_id, duty, duty_start_date, duty_end_date, style_color, last_update_date, last_updated_by, creation_date, created_by, freight, freight_duty
                                                        , request_id)
                SELECT operating_unit, country_of_origin, primary_duty_flag,
                       oh_duty, oh_nonduty, additional_duty,
                       inventory_org, inventory_item_id, duty,
                       duty_start_date, duty_end_date, style_color,
                       last_update_date, last_updated_by, creation_date,
                       created_by, freight, freight_duty,
                       gn_request_id
            ...
            write_log_prc (
                   SQL%ROWCOUNT
                || ' Rows Inserted Into XXDO.XXDO_INVVAL_DUTY_COST_BKP Table');
        ...
        EXCEPTION
            ...
            THEN
                ln_ins_cnt   := 0;
                write_log_prc (
                    'Failed to Insert Records in Backup Table XXDO.XXDO_INVVAL_DUTY_COST_BKP');
        END;
        ...
        BEGIN
            ...
            write_log_prc (
                   SQL%ROWCOUNT
                || ' Rows Deleted from XXDO.XXDO_INVVAL_DUTY_COST Table');
        ...
        EXCEPTION
            ...
            THEN
                write_log_prc (
                    'Failed to Delete Records from XXDO.XXDO_INVVAL_DUTY_COST');
        END;
    EXCEPTION
        WHEN OTHERS
        THEN
    ...
    BEGIN
        ...
        BEGIN
            ...
            SELECT directory_path
              ...
              FROM dba_directories
             WHERE 1 = 1 AND directory_name LIKE 'XXD_CST_DUTY_ELE_INB_DIR';
        EXCEPTION
            WHEN OTHERS
            THEN
                lv_inb_directory_path   := NULL;
        ...
        BEGIN
            ...
            SELECT directory_path
              ...
              FROM dba_directories
             WHERE 1 = 1 AND directory_name LIKE 'XXD_CST_DUTY_ELE_ARC_DIR';
        EXCEPTION
            WHEN OTHERS
            THEN
                lv_arc_directory_path   := NULL;
        ...
        LOOP
            ...
            IF ln_file_exists = 0
            THEN
                load_file_into_tbl_prc (pv_table => 'XXD_CST_DUTY_ELE_INB_STG_TR_T', pv_dir => 'XXD_CST_DUTY_ELE_INB_DIR', pv_filename => lv_file_name, pv_ignore_headerlines => 1, pv_delimiter => '|', pv_optional_enclosed => '"'
        ...
        apps.fnd_file.put_line (
            apps.fnd_file.output,
            '                                                                      Deckers TRO Inbound Duty Elements Upload');
        apps.fnd_file.put_line (apps.fnd_file.output, '');
        apps.fnd_file.put_line (
            apps.fnd_file.output,
            'Date:' || TO_CHAR (SYSDATE, 'DD-MON-YYYY HH24:MI:SS'));
        ...
        BEGIN
            ...
            apps.fnd_file.put_line (
                apps.fnd_file.output,
                   ' Number of Rows Successfully Inserted in Process Staging Table        - '
                || ln_ele_rec_total);
            apps.fnd_file.put_line (
                apps.fnd_file.output,
                '*************************************************************************************************');
        ...
        THEN
            ...
            THEN
                ...
                lv_message1   :=
                    ...
                    || CHR (10)
                    || CHR (10)
                    || 'Please Find the Attached Deckers TRO Inbound Duty Elements Exception Report. '
                    || CHR (10)
                    || CHR (10)
                    || lv_message2
                    || CHR (10)
                ...
                xxdo_mail_pkg.send_mail (
                    ...
                    pv_ccrecipients   => lv_ccrecipients,
                    pv_subject        =>
                        'Deckers TRO Inbound Duty Elements Exception Report',
                    pv_message        => lv_message1,
