--
-- XXD_WMS_GENERATE_LPN_PKG  (Package Body) 
--
/* Formatted on 4/26/2023 4:26:53 PM (QP5 v5.362) */
CREATE OR REPLACE PACKAGE BODY APPS."XXD_WMS_GENERATE_LPN_PKG"
AS
    ...
    IS
        ...
        --BT Tech Team
        CURSOR c1 IS
            SELECT DISTINCT org_id, lpn, lpn_length
              FROM xxd_conv.xxd_inv_item_onhand_lpn_stg_t
             --         Start Change for LPNs created by Onhand conversions 29-FRB-2016 By BT Team
             WHERE     lpn NOT IN
                           (SELECT license_plate_number FROM apps.wms_license_plate_numbers)
                   AND TRANSLATE (lpn, ' 0123456789', ' ') IS NULL;
    --         End Change for LPNs created by Onhand conversions 29-FRB-2016 By BT Team
    BEGIN
        fnd_file.put_line (
            fnd_file.LOG,
        ...
        fnd_global.apps_initialize (1132, 21676, 385);
        ...
        LOOP
            ...
            wms_container_pub.generate_lpn (
                ...
                p_total_length       => l_length--p_total_length  Necessary for leading Zeros
                                                );
    ...
    EXCEPTION
        ...
        THEN
            fnd_file.put_line (
                fnd_file.LOG,
                'Error in xxd_wms_generate_lpn_pkg' || SQLERRM);
    END;
