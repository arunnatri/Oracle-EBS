--
-- XXD_VT_ICS_INVOICES_PKG  (Package Body) 
--
/* Formatted on 4/26/2023 4:27:01 PM (QP5 v5.362) */
CREATE OR REPLACE PACKAGE BODY APPS."XXD_VT_ICS_INVOICES_PKG"
IS
    /******************************************************************************
       ...
       2.1    16-JAN-2020   Srinath    Macau PhaseII - UAT defect# 1879
       2.3    18-MAR-2020   Showkath   Macau PhaseII - New request to get the ship to country from SO
       2.4    25-MAR-2020   Showkath   Macau PhaseII - New logic to derive the Tax Codes
       2.5    02-APR-2020   Showkath   Macau PhaseII - New logic to derive final selling company for Retail customers.
       2.6    22-OCT-2020   Showkath   CCR0008987
       3.0    10-DEC-2020   Srinath    CCR0008507 - MTD Project
       3.1    02-APR-2021   Srinath    CCR0009103 - MTD Project PIII
    ******************************************************************************/
                          ...
                          FROM (SELECT attribute1 template_type, attribute2 language, attribute3 seller_company,
                                 ...
                                 WHERE     1 = 1
                                       ...
                                       AND ffvs.flex_value_set_id =
                                           ffvl.flex_value_set_id
                                       AND NVL (ffvl.enabled_flag, 'Y') = 'Y'
                                       -- Start of Change as per CCR0009103
                                       AND pd_invoice_date BETWEEN NVL (
                                                                       ffvl.start_date_active,
                                                                       pd_invoice_date)
                                                               AND NVL (
                                                                       ffvl.end_date_active,
    ...
    BEGIN
        ...
        --                        zx_exceptions ze
        --                    WHERE
        --                        ze.tax_rate_code = oola.tax_code
        --                        AND ze.product_id = oola.inventory_item_id
        --                );
        --
        --        EXCEPTION
        ...
        ELSE
            ...
            THEN
                BEGIN
                    /*SELECT DISTINCT tax_code
                      ...
                      FROM (SELECT tax_code
                              FROM (SELECT ffvs.flex_value_set_name,
                                           ffvl.attribute1 ou_id,
                                           UPPER (ffvl.attribute2) ship_to_equal,
                                           UPPER (ffvl.attribute3) ship_to_unequal,
                                           UPPER (ffvl.attribute4) ship_from_equal,
                                           UPPER (ffvl.attribute5)
                                              ship_from_unequal,
                                           ffvl.attribute6 channel,
                                           ffvl.attribute7 tax_code,
                                           ffvl.attribute8 ship_to_region
                             ...
                             WHERE     1 = 1
                                   AND ou_id = pv_org_id
                                   --                                AND ship_to_region = pv_ship_to_reg
                                   AND UPPER (channel) = UPPER (pv_channel)
                                   AND CASE
                                          WHEN NVL (ps_ship_from, '111') =
                                                  NVL (ship_from_equal, '111')
                     ...
                     */
                    /*  AND (CASE
                              WHEN (   NVL (pv_ship_to, '111') =
                                          NVL (ship_to_equal, '111')
                                    OR     (    NVL (pv_ship_to_reg,
                                                     '111') =
                                                   NVL (
                                                      ship_to_equal,
                                                      '111')
                                            AND ship_to_equal = 'EU')
                                       AND (   ship_to_equal = 'NA'
                                            OR ship_to_unequal = 'NA'))
                              THEN
                                 NVL (ship_to_equal, '111')
                              WHEN ( (    NVL (pv_ship_to, '111') <>
                                             NVL (ship_to_unequal,
                                                  '111')
                                      AND ship_to_unequal NOT IN
                                             ('NA', 'EU')
                                      AND (   ship_to_equal = 'NA'
                                           OR ship_to_unequal = 'NA')) -- OR (DECODE(:pv_ship_to,ship_to_unequal,ship_to_unequal,NVL(:pv_ship_to_reg, '111')) <> ship_to_unequal AND ship_to_unequal <> 'NA' ) AND (ship_to_equal = 'NA' OR ship_to_unequal = 'NA')
                                                                      )
                              THEN
                                 NVL (ship_to_unequal, '111')
                           END =
                              CASE
                                 ...
                                 WHEN ( (    NVL (pv_ship_to, '111') <>
                                         ...
                                         AND (   ship_to_equal = 'NA'
                                              OR ship_to_unequal =
                                                    'NA')) --OR (DECODE(:pv_ship_to,ship_to_unequal,ship_to_unequal,NVL(:pv_ship_to_reg, '111')) <> ship_to_unequal AND ship_to_unequal <> 'NA' ) AND (ship_to_equal = 'NA' OR ship_to_unequal = 'NA')
                                                          )
                                 THEN
                                    NVL (ship_to_unequal, '111')
                              END)
                          ...
                          END =
                             CASE
                                ...
                                THEN
                                   NVL (ship_from_unequal, '111')
                             END
                      AND (       pv_ship_to_reg = ship_to_equal
                              AND ship_to_unequal NOT LIKE
                                     '%' || pv_ship_to || '%'
                              AND (    ship_to_equal <> 'NA'
                                   AND ship_to_unequal <> 'NA')
                           OR (    pv_ship_to_reg <> ship_to_unequal
                               AND (   ship_to_equal = 'NA'
                                    OR ship_to_unequal = 'EU'))));*/
                    -- 2.4 changes start
                    SELECT DISTINCT tax_code
                      INTO x_tax_code
                      FROM (SELECT tax_code
    ...
    BEGIN
        ...
        RETURN lv_legal_text;
    /*IF p_type = 'INV' AND p_lang = 'EN' AND p_ComSeg1 IS NOT NULL AND p_ComSeg2 IS NOT NULL AND
       p_ComSeg1 IN ('110') and p_ComSeg2 IN ('140','270','230','590')
    THEN
       BEGIN
         SELECT  bill_to_country
           INTO  lv_country1
       ...
       BEGIN
         SELECT  bill_to_country
           INTO  lv_country2
           FROM  xxcp_address_details
          WHERE  tax_registration_id = p_tax2;
       EXCEPTION
           WHEN OTHERS
           THEN
              lv_country2 := NULL;
       ...
       IF   p_ComSeg1 = '110'
       THEN
          IF p_ComSeg2 = '140' AND upper(lv_country1) IN ('UNITED KINGDOM','NETHERLANDS')
          THEN
             lv_legal_text := 'Subject to UK VAT';
          ELSIF p_ComSeg2 IN ('270','300','460','550') AND upper(lv_country1) IN ('UNITED KINGDOM','NETHERLANDS')
          THEN
             lv_legal_text := 'Intra-community VAT exempt supply of goods';
          ELSIF p_ComSeg2 IN ('230') AND upper(lv_country1) IN ('UNITED KINGDOM','NETHERLANDS')
          THEN
             lv_legal_text := 'Subject to NL VAT';
          ELSIF p_ComSeg2 IN ('590') AND upper(lv_country1) IN ('UNITED KINGDOM','NETHERLANDS')
          THEN
             lv_legal_text := 'Zero rated export';
          ELSE
             lv_legal_text := NULL;
          END IF;
       ELSE
       ...
       END IF;
     RETURN lv_legal_text;
    ELSIF p_type = 'NONINV' AND p_lang = 'EN' AND p_ComSeg1 IS NOT NULL AND p_ComSeg2 IS NOT NULL AND
          p_ComSeg1 IN ('110','140','230','270','300','460','550') and p_ComSeg2 IN ('110','140','230','270','300','460','550')
    THEN
      lv_legal_text := 'Reverse charge applies (services)';
      RETURN lv_legal_text;
    ELSE
      RETURN NULL;
    END IF;*/
    ...
    FUNCTION get_nonmat_tax_stmt (p_header_id NUMBER)
        RETURN VARCHAR2
    IS
        lv_tax_statement   VARCHAR2 (4000);
    BEGIN
        BEGIN
            SELECT LISTAGG (tax_description, ',') WITHIN GROUP (ORDER BY tax_description)
              INTO lv_tax_statement
              FROM (SELECT DISTINCT ffvl.attribute2 tax_description
                      FROM apps.fnd_flex_value_sets fvs, apps.fnd_flex_values_vl ffvl, apps.xxcp_ic_inv_lines xil
                     WHERE     fvs.flex_value_set_id = ffvl.flex_value_set_id
                           AND fvs.flex_value_set_name LIKE
                                   'XXD_VT_MTD_TAX_STMT_VS'
                           ...
                           AND xil.ic_tax_code = ffvl.attribute1
                           AND xil.attribute10 <> 'T'
                           AND ffvl.attribute1 IS NOT NULL
                           AND xil.invoice_header_id = p_header_id);
        EXCEPTION
            WHEN OTHERS
            THEN
                lv_tax_statement   := NULL;
        END;

        RETURN lv_tax_statement;
    END get_nonmat_tax_stmt;
    ...
    IS
        ...
        lv_nat_account       VARCHAR2 (100);
        ln_src_assign_id     NUMBER;
    BEGIN
        ln_source_id         := NULL;
        lv_transaction_ref   := NULL;
        lv_nat_account       := NULL;
        ln_src_assign_id     := NULL;

        IF pn_src_assign_id IS NULL
        THEN
            BEGIN
                SELECT DISTINCT source_assignment_id
                  INTO ln_src_assign_id
                  FROM xxcp_ic_inv_lines
                 WHERE invoice_header_id = pn_inv_header_id;
            EXCEPTION
                WHEN OTHERS
                THEN
                    ln_src_assign_id   := NULL;
        ...
        IF NVL (pn_src_assign_id, ln_src_assign_id) IS NOT NULL
        THEN
            BEGIN
                SELECT source_id
                  INTO ln_source_id
                  FROM apps.xxcp_source_assignments
                 WHERE source_assignment_id =
                       NVL (pn_src_assign_id, ln_src_assign_id);
            EXCEPTION
                WHEN OTHERS
                THEN
                    ln_source_id   := NULL;
            ...
            THEN
                ...
                IF lv_transaction_ref IS NOT NULL
                THEN
                    BEGIN
                        SELECT DISTINCT xph.segment6
                          INTO lv_nat_account
                          FROM xxcp_gl_interface xgi, xxcp_process_history xph
                         WHERE     1 = 1
                               AND xgi.vt_transaction_ref =
                                   lv_transaction_ref
                               AND xgi.vt_interface_id = xph.interface_id
                               AND xph.transaction_class NOT LIKE '%TAX%'
                               --AND xph.rule_id in (58,213)
                               AND EXISTS
                                       (SELECT 1
                                          FROM apps.fnd_flex_value_sets ffvs, apps.fnd_flex_values_vl ffvl
                ...
                RETURN lv_nat_account;
            ELSIF ln_source_id IN (23, 41, 33,
                                   6)
            THEN
                BEGIN
                    SELECT DISTINCT transaction_ref
                ...
                THEN
                    BEGIN
                        SELECT DISTINCT xgi.segment6
                          INTO lv_nat_account
                          FROM xxcp_gl_interface xgi
                         WHERE     1 = 1
                               AND xgi.vt_transaction_ref =
                                   lv_transaction_ref
                               AND vt_source_assignment_id =
                                   NVL (pn_src_assign_id, ln_src_assign_id);
                    EXCEPTION
                        WHEN OTHERS
                        THEN
    ...
    BEGIN
                     ...
                     XMLAGG (
                         XMLELEMENT (
                             ...
                             XMLELEMENT ("COMPANY_TAX_REG_ID",
                                         a.comp_tax_reg_id),
                             XMLELEMENT ("COMPANY_TAX_REF", c.attribute4) -- added as per defect 1879
                                                                         --,XMLElement("COMPANY_TAX_REF",a.comp_tax_ref)
                                                                         --,XMLElement("COMPANY_TAX_REF",DECODE(get_source(a.header_id),'Material Transactions',Get_comp_tax_reg('INV',a.att2,a.comp_tax_reg_id,a.header_id),a.comp_tax_ref))
                                                                         --Get_comp_tax_reg('INV',a.att2,a.comp_tax_reg_id,a.header_id),a.comp_tax_ref)) -- Commented as per defect 1879
                                                                         ,
                             XMLELEMENT ("COMPANY_DESC", a.comp_desc) --,XMLElement("BILL_TO_ADDRESS",a.BILL_TO_ADDRESS)
                                                                     --,XMLElement("BILL_TO_ADDRESS",Get_Address(c.invoice_address_id, 'B'))
                                                                     ,
                             XMLELEMENT ("LANG", get_lang (a.att2)),
                             XMLELEMENT ("SOLD_BY_OU",
                                         get_ou_country (a.att2, 'OU')),
                             XMLELEMENT ("SOLD_TO_OU",
                                         get_ou_country (a.att3, 'OU')) --,XMLELEMENT("SHIP_FROM",get_ou_country(a.att2,'COUNTRY'))
                                                                       --CCR0007979 changes start
                                                                       --,XMLELEMENT("SHIP_FROM",DECODE(get_source(a.header_id),'Material Transactions', get_ship_from_country (a.header_id),get_ou_country(a.att2,'COUNTRY')))
                                                                       ,
                             XMLELEMENT ("SHIP_FROM",
                                         get_ship_from (a.header_id, a.att3)) --,XMLELEMENT("SHIP_TO",get_ou_country(a.att3,'COUNTRY'))
                                                                             /*,XMLELEMENT("SHIP_TO", CASE WHEN get_source(a.header_id) = 'Material Transactions' AND XXD_VT_ICS_INVOICES_PKG.get_mmt_type(a.header_id) = 'Internal Shipments'
                                                                                                    THEN XXD_VT_ICS_INVOICES_PKG.get_ship_to_country (a.header_id)
                                                                                                    ELSE get_ou_country(a.att3,'COUNTRY') END)*/
                                                                             ,
                             XMLELEMENT ("SHIP_TO",
                                         get_ship_to (a.header_id, a.att3)) --CCR0007979 changes end
                                                                           /*,XMLELEMENT("BILL_TO_ADDRESS",CASE WHEN a.att2 = '100' AND a.att3 in ('140','270','550','460','300')
                                                                                                              AND get_source(a.header_id) = 'Material Transactions'
                                                                                                              AND upper(get_country(a.comp_tax_reg_id)) = 'NETHERLANDS'
                                                                                                              THEN get_const_address ELSE Get_Address(c.invoice_address_id, 'B') END)*/
                                                                           ,
                             XMLELEMENT (
                                 "BILL_TO_ADDRESS",
                                 Get_Address (c.invoice_address_id, 'B')),
                             /*XMLELEMENT ("BILL_TO_ADDRESS",
                                         get_const_address (a.att2,
                                                            a.att3,
                                                            a.header_id,
                                                            a.comp_tax_reg_id,
                                                            c.invoice_address_id)),*/
                             ...
                             XMLELEMENT (
                                 ...
                                 DECODE (
                                     ...
                                     'Material Transactions', get_legal_text (
                                                                  ...
                                                                  'EN',
                                                                  a.header_id),
                                     get_legal_text (a.att2, a.att3, a.comp_tax_reg_id, a.cust_tax_reg_id, 'NONINV', 'EN'
                                                     , a.header_id))) --,XMLELEMENT("GET_TEXT_VAT",DECODE(get_source(a.header_id),'Material Transactions',get_legal_text_mt(a.att2,a.att3,a.comp_tax_reg_id,a.cust_tax_reg_id,'INV','EN',a.header_id,get_cust_classification(b.att1))
                                                                     --,get_legal_text_mt(a.att2,a.att3,a.comp_tax_reg_id,a.cust_tax_reg_id,'NONINV','EN',a.header_id,get_cust_classification(b.att1)))) -- CCR0007979 - Macau changes
                                                                     ,
                             XMLELEMENT ("GET_NMT_TEXT",
                                         get_nonmat_tax_stmt (a.header_id)),
                             ...
                             XMLAGG (XMLELEMENT (
                                         ...
                                         XMLELEMENT ("TAX_CODE", b.tax_code) -- CCR0007979 - Macau changes start
                                                                            --,xmlelement("TAX_CODE1",get_tax_codes(get_sold_to(get_ou_country(a.att3,'OU')),NVL(b.tax_rate,0),get_ship_from_country_code(get_ship_to(a.header_id,a.att3)),get_eu_non_eu(get_ship_to(a.header_id,a.att3)),get_ship_from_country_code(get_ship_from(a.header_id,a.att3))))
                                                                            --,xmlelement("TAX_DESC",decode((get_tax_codes(get_sold_to(get_ou_country(a.att3,'OU')),NVL(b.tax_rate,0),get_ship_from_country_code(get_ship_to(a.header_id,a.att3)),get_eu_non_eu(get_ship_to(a.header_id,a.att3)),get_ship_from_country_code(get_ship_from(a.header_id,a.att3)))),'T0','Zero rated','T1','Standard rated','T2','Out of scope'))
                                                                            ,
                                         XMLELEMENT ("NEW_TAX_RATE",
                                                     NVL (b.tax_rate, 0)) --,XMLELEMENT("GET_TEXT_VAT",DECODE(get_source(a.header_id),'Material Transactions',get_legal_text_mt(a.att2,a.att3,get_cust_classification(b.att1,'ORG'),a.comp_tax_reg_id,a.cust_tax_reg_id,'INV','EN',a.header_id,get_cust_classification(b.att1,'CLASS'))
                                                                         --,get_legal_text_mt(a.att2,a.att3,get_cust_classification(b.att1,'ORG'),a.comp_tax_reg_id,a.cust_tax_reg_id,'NONINV','EN',a.header_id,get_cust_classification(b.att1,'CLASS')))) -- CCR0007979 - Macau changes
                                                                         -----------------------
                                                                         ,
                                         /*XMLELEMENT (
                                            "TAX_CODE_INV",
                                            get_tax_codes_vt_new (
                                               get_sold_to (get_ou_country (a.att2, 'OU')),
                                               UPPER (
                                               ...
                                               get_eu_non_eu (
                                                  --get_ship_to (a.header_id, a.att3)--2.3
                                                  get_so_ship_to (b.att1, a.header_id) -- 2.3
                                                                                      ),
                                               UPPER (
                                                  get_ship_from_country_code (
                                         ...
                                         XMLELEMENT (
                                             "TAX_CODE_INV",
                                             get_tax_codes_vt_new (
                                                 (DECODE (get_source (a.header_id), 'Material Transactions', 'INV', 'NONINV')),
                                                 get_lang (a.att2),
                                                 a.att2,
                                                 a.att3,
                                                 ...
                                                 UPPER (
                                                     get_ship_from_country_code (
                                                         get_ship_from (
                                                             a.header_id,
                                                             a.att3))),
                                                 b.invoice_date -- Added as per CCR0009103
                                                               )),
                                         XMLELEMENT (
                                             "TAX_STMT_INV",
                                             get_tax_stamt_vt_new (
                                                 ...
                                                 UPPER (
                                                     get_ship_from_country_code (
                                                         get_ship_from (
                                                             a.header_id,
                                                             a.att3))))) -- CCR0007979 - Macau changes end
                                                                        ,
                                         ...
                                         XMLELEMENT (
                                             ...
                                             get_account_fnc (
                                                 a.header_id,
                                                 c.source_assignment_id)), -- Added as per CCR0008507
                             ...
                             XMLELEMENT (
                                 ...
                                 (  SELECT XMLAGG (
                                               XMLELEMENT (
                                                   "TOTAL",
                                                   XMLFOREST (   --c.TAX_CODE,
                                                       c.ic_currency curr, --c.tax_rate,
                                                       --SUM(c.TAX_AMOUNT) tax_amt,
                                                       SUM (
                             ...
                             XMLELEMENT (
                                 ...
                                 (  SELECT XMLAGG (
                                     ...
                                     WHERE     c.invoice_header_id = d.header_id
                                           AND c.invoice_header_id = a.header_id
                                           AND c.att10 <> 'T'
                                  GROUP BY c.tax_code, c.ic_currency, c.tax_rate)) -- CCR0007979 - Macau changes start
                                                                                  ,
                             XMLELEMENT (
                                 "NEW_TAX_LOGIC",
                                 (  SELECT XMLAGG (
                                               XMLELEMENT (
                                                   ...
                                                   XMLFOREST (
                                                       ...
                                                       /*get_tax_codes_vt_new (
                                                          get_sold_to (
                                                             get_ou_country (d.att2, 'OU')),
                                                          UPPER (
                                                             get_ship_from_country_code (
                                                                -- get_ship_to (d.header_id,
                                                                -- d.att3
                                                                get_so_ship_to (
                                                                   c.att1,
                                                                   d.header_id)   --2.3
                                                       ...
                                                       DECODE (
                                                          ...
                                                          'T0', 'Zero rated',
                                                          'T1', 'Standard rated',
                                                          'T2', 'Out of scope') TAX_DESC_NEW --,get_tax_codes_vt_new(get_sold_to(get_ou_country(d.att2,'OU')),UPPER(get_ship_from_country_code(get_ship_to(d.header_id,d.att3))),get_eu_non_eu(get_ship_to(d.header_id,d.att3)),UPPER(get_ship_from_country_code(get_ship_from(d.header_id,d.att3))),get_cust_classification(c.att1,'CLASS',d.header_id),get_so_line_id(c.att1,d.header_id)) TAX_CODE_NEW
                                                                                            --,decode((get_tax_codes_vt_new(get_sold_to(get_ou_country(d.att2,'OU')),UPPER(get_ship_from_country_code(get_ship_to(d.header_id,d.att3))),get_eu_non_eu(get_ship_to(d.header_id,d.att3)),UPPER(get_ship_from_country_code(get_ship_from(d.header_id,d.att3))),get_cust_classification(c.att1,'CLASS',d.header_id),get_so_line_id(c.att1,d.header_id))),'T0','Zero rated','T1','Standard rated','T2','Out of scope') TAX_DESC_NEW
                                                                                            --,SUM(c.quantity)total_qty, SUM(c.Unit_Price)tot_price
                                                       ,*/
                                                       --2.4 changes start
                                                       get_tax_codes_vt_new (
                                                           (DECODE (get_source (d.header_id), 'Material Transactions', 'INV', 'NONINV')),
                                                           get_lang (d.att2),
                                                           d.att2,
                                                           d.att3,
                                                           get_cust_classification (
                                  ...
                                  GROUP BY NVL (c.tax_rate, 0),
                                           ...
                                           DECODE (
                                               ...
                                               'T0', 'Zero rated',
                                               'T1', 'Standard rated',
                                               'T2', 'Out of scope'))) -- 2.4 changes end
        ...
        GROUP BY a.header_id, c.invoice_address_id, c.attribute4 -- Added as per defect 1879
                 ...
                 a.att20, c.invoice_class, c.source_assignment_id -- Added as per CCR008507
        --,b.att1
        ORDER BY b.invoice_number, TO_NUMBER (b.line_no);
